 ...
 *
 */
class Hcma extends API_Controller 
{
	...
	//HCMA apis
    public function hcma_basic_detail_post() {
    ...
    public function hcma_personal_detail_post() {
        $applicant_personal_det_table = APPLICANT_PERSONAL_DET_TABLE;
        ...
        // course_pref
        $course_res = array();         
        if($course_pref && is_numeric($course_pref)) {
                $_SERVER["REQUEST_METHOD"] = "POST";
                $_POST = array();
                $_POST['active'] = TRUE;
                if($course_prefer_id) {
        ...
        if($applicant_personal_det_res){
            $wizard_update = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_hcma , FORM_WIZARD_PERSONAL_ID);
        }
        $this->response ($applicant_personal_det_res , API_Controller::HTTP_OK);
    }
    
        public function hcma_parent_detail_post()
        {
            $app_form_id_hcma = APP_FORM_ID_HCMA;
        ...
        public function hcma_addressdet_post()
        {
        ...
        public function hcma_academicdet_post()
        {
            $user_data = $this->post();
            $table_schema = SCHEMA_ADMISSION;
            $applicant_personal_det_table = APPLICANT_PERSONAL_DET_TABLE;
            ...
            $sslc_result_status_val=$user_data['sslc_result_status_val'];
           ...
           // $_POST['updated_by'] = $applicant_personal_det_id;
            ...
            $this->form_validation->set_rules('institute_name_tenth', 'Institute Name','trim|required');
            $this->form_validation->set_rules('board_tenth', 'Board','trim|required');
            if($cur_qual_completed == 't') {
                $this->form_validation->set_rules('marking_scheme_tenth', 'Marking Scheme','trim|required');
                $this->form_validation->set_rules('cgpa_tenth', 'Obtained Percentage/CGPA','trim|required');
                $this->form_validation->set_rules('yr_of_passing_tenth', 'Year of Passing','trim|required');
            }
            ...
            //if ($marking_scheme_tenth!='') { 
                $applicant_schooling_det_columns.=',marking_scheme_id';
           // }
           // if (is_numeric($cgpa_tenth)) {
                $applicant_schooling_det_columns.=',mark_percentage';
           // }
            //set result declared and is current qualify field
            $tenth_declared=$tenth_qualify=$twelfth_qualify='t';
            if(isset($sslc_result_status_val) && $sslc_result_status_val!=""){
                $tenth_declared= ($sslc_result_status_val==1) ? 't' :'f';
                $twelfth_qualify=$twelfth_declared='f';
            }
            if(isset($current_qualification_status) && $current_qualification_status!=""){
                $tenth_declared= 't';
                $twelfth_declared= ($current_qualification_status==1) ? 't' :'f';
            }
            if(empty($schooling_id_tenth)) {             
             $_POST['schooling_id'] = SCHOOLING_ID_TENTH;
            }  
            //echo gettype($sslc_result_status_val);
            if(isset($sslc_result_status_val) && ($sslc_result_status_val==="0")){
                $cgpa_tenth=NULL;
                $yr_of_passing_tenth=NULL;
                $roll_no_tenth=NULL;               
                $marking_scheme_tenth=NULL;
            }
            ...
            $_POST['result_declared'] = $tenth_declared;
            $_POST['is_curr_qualify'] = $tenth_qualify;
            ...
            //for 12th
            if($current_qualification_status!=""){
                ...
                $cgpa_twelfth = ($user_data['cgpa_twelfth']!='' && $user_data['cgpa_twelfth']!='null') ?$user_data['cgpa_twelfth'] :'';
               ...
               // if ($stream_name!='') {
                ...
                if($current_qualification_status==0){
                    $cgpa_twelfth=NULL;
                ...
                if(isset($sslc_result_status_val) && $sslc_result_status_val!=""){
                    ...
                    $where_hsc_gno = array();
                    $where_hsc_gno['applicant_personal_det_id'] = $applicant_personal_det_id;
                    $where_hsc_gno['applicant_appln_id'] = $applicant_appln_id;
                    $where_hsc_gno['schooling_id'] = SCHOOLING_ID_TWELFTH;
                    $where_hsc_gno['table_schema'] = SCHEMA_ADMISSION;
                    $this->common->common_delete_new ( $tbl_schema_both , $where_hsc_gno );
                }
            }
       ...
       public function hcma_declaration_post() {
           $app_form_id_hcma = APP_FORM_ID_HCMA;
           $applicant_personal_det_table = APPLICANT_PERSONAL_DET_TABLE;
           $applicant_appln_det_table = APPLICANT_APPLN;
           $table_schema = SCHEMA_ADMISSION;
           ...
           if($applicant_personal_det_res){
               $wizard_update = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_hcma , FORM_WIZARD_UPLOAD_ID);
           }
           $this->response ($applicant_personal_det_res , API_Controller::HTTP_OK);
       }
       
       
       public function hcma_preference_get() {
           $table_schema = SCHEMA_ADMISSION;
           ...
           //$arguments=array($grade_id,$campus_id,$course_id,$degree_id,$spec_id,$form_id);           
