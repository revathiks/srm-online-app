 <script type="text/javascript">
	...
	$(function() {
	 ...
	 //  });
	  ...
	  $("form[name='loginForm']").parsley().on('field:validated', function() {
	  ...
	  .on('form:submit', function() {
		...
		var userData = $("#loginForm").serialize();
		$.ajax({
			url: '<?php echo base_url(); ?>crm-admin/login/',
			method: "POST",
			data: userData,
			dataType: 'json',
			cache: false,
			success: function(returndata) {
				...
				if(stringifyJSONLoginParseStatus=='<?php echo SUCCESS_LOGIN_STATUS; ?>'){
					...
					$("#loginSubmit").attr('disabled','true');
					setTimeout(function(){
						$('#loginSubmit').html('<?php echo LOGIN_CRM_BUTTON; ?>');
						$('#msgs').html("<div class='alert alert-danger' style='text-align: left;'>"+returndata.msg+"<button type='button' class='close'   data-dismiss='alert'>&times;</button></div>");
