<?php 
$app_form_id_bba = APP_FORM_ID_BBA;
...
if($applicant_application_details_list) {
    foreach($applicant_application_details_list as $k=>$v) {
        ...
        if($app_form_id_bba == $appln_form_id) {
...
if(!empty($applicant_schooling_list)){  
    if(!empty($result_declared_twelfth) && $result_declared_twelfth == 't'){
        $tenthRequired=$twelfthRequired='true';
    }
}
if($upload_filelist) {
    foreach($upload_filelist as $doc){
        ...
        $parsley_required = (($document_id_photograph == $document_id)?'true':(($document_id_signature == $document_id)?'true':(($document_id_tenth_marksheet == $document_id)?$tenthRequired:(($document_id_twelvfth_marksheet == $document_id)?$twelfthRequired:''))));
...
<?php if($mode_edit) { ?>
  ...
  var url = "<?php echo base_url(); ?>bba/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
...
var app_form_id = '<?php echo $app_form_id_bba; ?>';
...
$(document).ready(function () {
    ...
    var settings = {
				 ...
				 if(currentIndex === 0) {	                 
					...
					if(currentIndex === 1) {
					  ...
					  if($('#main_div_social_status').is(':visible')){
				         ...
				         if(campus_pref_1.isValid()	&& pd_title.isValid()
        ...
        onStepChanged: function (event, currentIndex, priorIndex) { 
			...
			if(currentIndex == parseInt(total_wizard_Step - 1)){
				...
				if(mode_edit_url !='')
		        {
		         	enable_preview_btn(currentIndex,'<?php echo base_url();?>bba_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
		        }else{
				enable_preview_btn(currentIndex,'<?php echo base_url();?>bba_preview');
		        }
			}
			else{
				$("#previewbtn").remove();
	...
	$("#bba_form").steps(settings);
  	//setsave exit attr	
    $(document).on("click", "#save_btn" , function() {
    	 $("#bba_form").attr('isexit',1);
         $("#bba_form").steps('next');
    });
    
    $('.actions a').click(function(){        	 
      $("#bba_form").attr('isexit','');
    }); 
	//end set attr
  ...
  //to remove links from previous tabs a
    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0) && ($mode_edit == '')) { ?>
     $( ".steps li" ).each(function( index ) { 
      if(index<6){       	 
   	  $("#bba_form-t-"+index).removeAttr("href");
     }			   
   	});
    <?php } ?>	
		...
		getSelect2('campus_pref_1','<?php echo base_url("course_preference"); ?>?is_lookup_master=1&is_campus=1&grade_id=1&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select Campus Perferences 1', formatRepoCommon,no_campus_msg, false, formatRepoSelection);
	    getSelect2('campus_pref_2','<?php echo base_url("course_preference"); ?>?is_lookup_master=1&is_campus=1&grade_id=1&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select Campus Perferences 2', formatRepoCommon,no_campus_msg, false, formatRepoSelection);
	    getSelect2('campus_pref_3','<?php echo base_url("course_preference"); ?>?is_lookup_master=1&is_campus=1&grade_id=1&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select Campus Perferences 3', formatRepoCommon,no_campus_msg, false, formatRepoSelection);
	    ...
	    $('#campus_pref_1').on('change',function() {	    	 
		 ...
		 $('#campus_pref_3').on('change',function() {
			 ...
			 {
                $("#custom-field-error").html("<?php echo REQD_SELECT_CAMPUS_PREFERENCE2_MSG;?>");
                $("#campus_pref_3").val('');
	...
	<?php			
			if(!empty($applicant_schooling_list)){
			    ...
			    if(!empty($result_declared_twelfth)){
			        $cur_qual_completed=$result_declared_twelfth;
			     }
	  ...
	  $('#pd_nationality').on('change',function() {
		...
		setTimeout(function() { 
			if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
				...
				if(mode_edit_url !='')
		        {
		         	enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>bba_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
		        }else{
				enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>bba_preview');
		        }
			}
		},1);
		...
		$("#academic_yr_passing").datepicker( {
			...
			var status=$('input[name=current_qualification_status]:checked').val();		
			if(status==2){
			var Yr=$("#academic_yr_passing").val();
                if(Yr!="" && Yr!="YYYY"){                
             	var addYr=2;
...
function campus_pref_change(currentcampus,reset1,reset2,campus_placeholder,choice) {
    ...
    if(notcampus2.length>0);
    {
		...
		$('#'+reset1).html(''); 		
		var no_campus_msg = "Sorry, Campus not available.";
		getSelect2(reset1,'<?php echo base_url("course_preference"); ?>?is_lookup_master=1&ecamp='+notcampusval2+'&is_campus=1&grade_id=1&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select Campus Perferences 2', formatRepoCommon,no_campus_msg, false, formatRepoSelection);
		if(choice==1){
			...
			$('#'+reset2).html('');
			var no_campus_msg = "Sorry, Campus not available.";		
			getSelect2(reset2,'<?php echo base_url("course_preference"); ?>?is_lookup_master=1&ecamp='+notcampusval2+'&is_campus=1&grade_id=1&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select Campus Perferences 3', formatRepoCommon,no_campus_msg, false, formatRepoSelection); 
	     }
	}
...
function upload_file(file_doc_type, upload_type) {
	...
	if(valid) {
		...
		if(file_doc_type=="tenth_marksheet") {
			parsley_required='<?php echo $tenthRequired;?>';
		}
		if(file_doc_type=="twelvfth_marksheet") {
			parsley_required='<?php echo $twelfthRequired;?>';
		}
