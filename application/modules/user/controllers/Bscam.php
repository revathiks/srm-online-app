...
class Bscam extends FrontendController
{
	//
    public $CI;
    ...
    public function bsc_hons_am_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		// Check user is exist
		...
		if ( $this->input->post () ){	
			...
			if($user_data['currentIndex']==0){
				$url = $api_urls['basic_detail_bsc_am'];
				// Method Type
				$method = 'POST';
                ...
                $response = array();
                $response['basic_details_bsc'] = $result;
			...
			if($user_data['currentIndex']==1){
				$url = $api_urls['personal_detail_bsc_am'];
				// Method Type
				$method = 'POST';
			...
			if($user_data['currentIndex']==2)
            {
                ...
                // parent detail
                $api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'bsc_parent_detail' ];
                $method = 'POST';
                //$userdata = $this->session_userdata();
				$userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
                ...
                $user_data['applicant_personal_det_id'] = $applicant_id;
                $user_data['applicant_login_id'] = $applicant_login_id;
                $user_data['app_form_id'] = APP_FORM_ID_BSCAM;
                list($result_token,$data) = $this->_check_token($user_data);  
                $addr_response = false;
                if($result_token['valid']=='true')
                {
			...
			if($user_data['currentIndex']==3){
				$api_urls = $this->config->item ( 'api_urls' );
				$url = $api_urls[ 'bsc_academic_detail' ];
				$method = 'POST';
				// $userdata = $this->session_userdata();
				$userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
			...
			if($user_data['currentIndex']==5){
				$api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'declaration_bscam' ];
                $method = 'POST';
				// $userdata = $this->session_userdata();
				$userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
			...
			$wizard_dt = $this->_get_wizard_details(APP_FORM_ID_BSCAM);
            ...
            $arr_wizard_id = array(FORM_WIZARD_BASIC_ID,FORM_WIZARD_PERSONAL_ID,FORM_WIZARD_PARENT_ID,FORM_WIZARD_ACADEMIC_ID,FORM_WIZARD_PAYMENT_ID,FORM_WIZARD_UPLOAD_DECLARATION_ID);
			...
			$this->data['site_title'] = $this->data['page_title'] = 'B.Sc (Hons) Aircraft Maintenance';
			$this->template('applications/bsc_hons_am', $this->data, true);
		}
    }
	...
	public function bscam_form_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		$this->is_exists_user_logged();
		/*CRM ADMIN Edit form start*/
		$this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
		$this->data['mode_edit'] = $mode;
		...
		$this->data['user_data'] = $this->user_details_data['user_details']['data'][0];
		$this->data['personal_detail_list'] = $this->personal_detail_list($applicant_id);
		$this->data['site_title'] = $this->data['page_title'] = 'B.Sc (Hons) Aircraft Maintenance Preview';

        // Display page with the template function from REST_Controller
        $this->template('applications/bsc_hons_am_form_preview', $this->data, true);
    }	

	public function get_spl_preferences_bsc()
    {
	...
	public function get_campus_bsc()
    {
		$prog_id = $this->input->get('prog_id');
		$keywords=$this->input->get('keywords');
		$query_data = array();
        ...
        $api_urls = $this->config->item ( 'api_urls' );
        $url = $api_urls['db_func_call_bsc_campus'];
        $query = http_build_query($query_data);
        $url = $url .'?'.$query;
        $method = 'GET';
        $userdata = $this->session_userdata();
