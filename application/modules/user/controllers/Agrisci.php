...
class Agrisci extends FrontendController
{
	//
    public $CI;
    ...
    public function Agrisci_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		...
		// Get Form ID
        $get_form_id = APP_FORM_ID_AGRISCI;
		// Get Applicant ID
		...
		if ( $this->input->post () ){	
			...
			if($user_data['currentIndex']==0){
				$url = $api_urls['basic_details_agrisci'];
				// Method Type
				$method = 'POST';
                ...
                $response = array();
                $response['basic_details_agrisci'] = $result;
			...
			if($user_data['currentIndex']==1){
				$url = $api_urls['personal_detail_agrisci'];
				// Method Type
				$method = 'POST';
				$userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
				$user_data['applicant_personal_det_id'] = $applicant_id;
				$user_data['pd_program_level_id'] = $user_data['pd_program_level'];
				$user_data['course_id'] = $user_data['pd_course_preference'];
				$user_data['choice_no'] = DEFAULT_CHOICE_NO;
			...
			if($user_data['currentIndex']==2)
            {
                ...
                // Parent detail
                $api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'agrisci_parent_detail' ];
                $method = 'POST';
				$userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
                $user_data['applicant_personal_det_id'] = $applicant_id;
			...
			if($user_data['currentIndex']==4){
				$api_urls = $this->config->item ( 'api_urls' );
				$url = $api_urls[ 'agrisci_academic_detail' ];
				$method = 'POST';
				$userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
				$user_data['applicant_personal_det_id'] = $applicant_id;
			...
			if($user_data['currentIndex']==6){
				$api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'declaration_agrisci' ];
                $method = 'POST';
				$userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
			...
			$wizard_dt = $this->_get_wizard_details(APP_FORM_ID_AGRISCI);
			...
			$this->data['site_title'] = $this->data['page_title'] = 'Agricultural Sciences Form';
			$this->template('applications/form_agrisci', $this->data, true); 
		}    
    }		

	// Get Program Level Of Agrisci
	public function get_agrsci_pgm_level()
    {
        ...
        $api_urls = $this->config->item ( 'api_urls' );
        $url = $api_urls['db_func_call_agrisci_pgm_level'];
	...
	// Get Course Preferences Of Agrisci
	public function get_agrisci_course_preference()
    {
		$prog_id = $this->input->get('prog_id');
		$keywords=$this->input->get('keywords');
		$query_data = array();
        ...
        $api_urls = $this->config->item ( 'api_urls' );
        $url = $api_urls['db_func_call_agrisci_course'];
		$query = http_build_query($query_data);
        $url = $url .'?'.$query;
        $method = 'GET';
        $userdata = $this->session_userdata();
	...
	/**
     * get current education qualification status
     *
     * @return show json response
     * @author
     */
    public function get_agrisci_cur_edu_status() {
        parent::_get_agrisci_cur_edu_status(false, $this->is_admin);
    }    
	
	// Agricultural Form Preview
	public function agrisci_form_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		$this->is_exists_user_logged();
		/*CRM ADMIN Edit form start*/
		$this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
		$this->data['mode_edit'] = $mode;
		...
		$this->data['user_data'] = $this->user_details_data['user_details']['data'][0];
		$this->data['personal_detail_list'] = $this->personal_detail_list($applicant_id);
		$this->data['site_title'] = $this->data['page_title'] = 'Agricultural Sciences Form Preview';

        // Display page with the template function from REST_Controller
        $this->template('applications/agriscience_form_preview', $this->data, true);
    }	
