<?php
$app_form_id_hspg_ee= APP_FORM_ID_HSPG_EE;
...
if($applicant_application_details_list) {
    foreach($applicant_application_details_list as $k=>$v) {
        ...
        if($app_form_id_hspg_ee == $appln_form_id) {
...
if($upload_filelist) {
    foreach($upload_filelist as $doc){
        ...
        $parsley_required = (($document_id_photograph == $document_id)?'true':(($document_id_signature == $document_id)?'true':(($document_id_graduation_marksheet == $document_id)?'false':'')));
...
<?php if($mode_edit) { ?>
  ...
  var url = "<?php echo base_url(); ?>hspg_ee/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
...
<?php } else { ?>
  var url_edit =  '';
  var url = "<?php echo base_url(); ?>hspg_ee";
...
var app_form_id = '<?php echo $app_form_id_hspg_ee; ?>';
...
$(document).ready(function () {	
 ...
 // Dynamically Get Total Wizard Steps
    ...
    var settings = {
				...
				// Step 1 form validation
				 if(currentIndex === 0) {	                 
					...
					if(currentIndex === 1) {
		                ...
		                var state_pref_3 = $('#state_pref_3').parsley();
		                var city_pref_3 = $('#city_pref_3').parsley();
                      ...
                      if($('#main_div_social_status').is(':visible')){
							...
							state_pref_3.validate();
							city_pref_3.validate();														
							return false;
					  	}
					}
					//end step2
        ...
        { 
			...
			if(currentIndex == parseInt(total_wizard_Step - 1)){
				...
				if(mode_edit_url !='')
		        {
		         enable_preview_btn(currentIndex,'<?php echo base_url();?>hspg_ee_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
		        }else{
				enable_preview_btn(currentIndex,'<?php echo base_url();?>hspg_ee_preview');
		        }
			}else{
				$("#previewbtn").remove();
			}
	...
	$("#hspg_ee_form").steps(settings);
  //setsave exit attr	
    $(document).on("click", "#save_btn" , function() {
    	 $("#hspg_ee_form").attr('isexit',1);
         $("#hspg_ee_form").steps('next');
    });
    
    $('.actions a').click(function(){        	 
      $("#hspg_ee_form").attr('isexit','');
    }); 
//end set attr
   //to remove links from previous tabs a
    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0) && ($mode_edit == '')) { ?>
     $( ".steps li" ).each(function( index ) { 
      if(index<6){       	 
   	  $("#hspg_ee_form-t-"+index).removeAttr("href");
     }			   
   	});
    <?php } ?>
    light_box_init();
          ...
          //fetch city
            ...
            if($applicant_city_prefer_details_list) {
                foreach($applicant_city_prefer_details_list as $k=>$v) {
                    $choice_no=$v['choice_no'];
                    $applicant_city_id[$choice_no] = $v['applicant_city_id'];
                    $applicant_city_city_id[$choice_no] = $v['city_id'];
                    $applicant_city_city_name[$choice_no] = $v['city_name'];
                    $applicant_city_choice_no[$choice_no] = $v['choice_no'];
                    $applicant_city_state_id[$choice_no] = $v['state_id'];
                    $applicant_city_state_name[$choice_no] = $v['state_name'];
                }
            }
	  ...
	  $('#pd_program').on('change',function() {
		...
		<?php			
		 if(!empty($qual_status_id)){		  
	      if($qual_status_id == HSPG_EE_EDU_PASSED) {
		         ?>
		...
		$('#academic_stream').on('change',function() {
		 ...
		 $('#state_pref_3').on('change',function() {
				$('#city_pref_3').attr('data-parsley-required',"false");
				var stateVal=$(this).val();					 
				if(stateVal>0)
				{
					$('#city_pref_3').attr('data-parsley-required',"true");
				}
	   });
		...
		// This Function for Payment option selection
			...
			setTimeout(function() { 
				if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
					...
					if(mode_edit_url !='')
			        {
			         enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>hspg_ee_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
			        }else{
					enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>hspg_ee_preview');
			        }
				}
			},1);
...
function upload_file(file_doc_type, upload_type) {
	...
	if(valid) {
		...
		if(file_doc_type=="graduation_marksheet"){
			...
			<?php //if($qual_status_id==HSPG_EE_EDU_PASSED) { ?>
