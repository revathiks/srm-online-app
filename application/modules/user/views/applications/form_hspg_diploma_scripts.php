<?php
$app_form_id_hspg_diploma= APP_FORM_ID_HSPG_DIPLOMA;
...
if($applicant_application_details_list) {
    foreach($applicant_application_details_list as $k=>$v) {
        ...
        if($app_form_id_hspg_diploma == $appln_form_id) {
...
<?php if($mode_edit) { ?>
  ...
  var url = "<?php echo base_url(); ?>hspg_diploma/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
...
<?php } else { ?>
  var url_edit =  '';
  var url = "<?php echo base_url(); ?>hspg_diploma";
...
var app_form_id = '<?php echo $app_form_id_hspg_diploma; ?>';
...
var sslcType='<?php echo SCHOOLING_ID_TENTH;?>';
var hscType='<?php echo SCHOOLING_ID_TWELFTH;?>';
var ugType='<?php echo GRADUATION_UG_ID;?>';
var pg_diploma='<?php echo PG_DIPLOMA_GRAD;?>';
var diploma='<?php echo DIPLOMA_GRAD;?>';
var certificate='<?php echo CERTIFICATE_GRAD;?>';
var ug='<?php echo UG_GRAD;?>';
var pg='<?php echo PG_GRAD;?>';
...
$(document).ready(function () {	
 ...
 // Dynamically Get Total Wizard Steps
    ...
    var settings = {
        ...
        onStepChanging: function (event, currentIndex, newIndex) { 
           if(currentIndex < newIndex) {
        	   var isexit = ($(this).attr('isexit'));
				// Step 1 form validation
				 if(currentIndex === 0) {	                 
					...
					if(currentIndex === 1) {
						...
						var pd_course_pref_3 = $('#course_pref_3').parsley();
                      ...
                      if($('#main_div_social_status').is(':visible')){
							...
							if(moveNxt){
					  		...
					  		if(pd_program.isValid() && pd_campus_pref_1.isValid() ){
					  			pd_course_pref_1.validate();
						  		pd_course_pref_2.validate();
						  		pd_course_pref_3.validate();
					  		}
							pd_program.validate();						
							pd_title.validate();
                ...
                var tenth_highest_edu = $('#tenth_highest_edu').parsley();
                var ug_highest_edu = $('#ug_highest_edu').parsley();
              ...
              var academic_reg_no = $('#academic_reg_no').parsley();
                      ...
                      var twelfth_highest_edu = $('#twelfth_highest_edu').parsley();
	               ...
	               if(grad_id== ug || grad_id== pg || grad_id==  pg_diploma){
	                ...
	                if( qualification==true && candidate_name.isValid() && academic_tenth_inst.isValid() && academic_board.isValid()  && academic_marking_scheme.isValid() && academic_obtain_cgpa.isValid() && academic_yr_passing.isValid() && academic_reg_no.isValid() && academic_twelfth_inst.isValid() && academic_twelfth_board.isValid() && academic_marking_scheme_twelfth.isValid() && academic_obtain_cgpa_twelfth.isValid() && academic_yr_passing_twelfth.isValid() && academic_reg_no_twelfth.isValid() &&  institute_name_ug.isValid() && university_ug.isValid() && user_marking_scheme_ug.isValid() && obtained_percentage_cgpa_ug.isValid() && yr_passing_ug.isValid() && register_no_ug.isValid() && digilocker_id.isValid() && tenth_highest_edu.isValid() && twelfth_highest_edu.isValid() && ug_highest_edu.isValid()) {
						...
						var grad_id_hidden_val=$('#grad_id_hidden').val();
						
	                	var moveNxt=0;   
	                    var user_data = 'current_education_qual_status='+current_education_qual_status+'&candidate_name='+candidate_name+'&academic_tenth_inst='+academic_tenth_inst_val+'&academic_tenth_board='+academic_board_val+'&other_tenth_board='+other_tenth_board_val+'&academic_tenth_msv='+academic_marking_scheme_val+'&academic_tenth_cgpa='+academic_obtain_cgpa_val+'&academic_tenth_yrp='+academic_yr_passing_val+'&academic_tenth_regno='+academic_reg_no_val+'&academic_twelfth_inst='+academic_twelfth_inst_val+'&academic_twelfth_board='+academic_twelfth_board_val+'&other_twelfth_board='+other_twelfth_board_val+'&academic_twelfth_msv='+academic_twelfth_marking_scheme_val+'&academic_twelfth_cgpa='+academic_twelfth_obtain_cgpa_val+'&academic_twelfth_yrp='+academic_twelfth_yr_passing_val+'&academic_twelfth_regno='+academic_twelfth_reg_no_val+'&grad_id_ug='+grad_id_ug+'&institute_name_ug='+institute_name_ug_val+'&university_ug='+university_ug_val+'&university_other_ug='+university_other_ug_val+'&marking_scheme_ug='+user_marking_scheme_ug_val+'&obtained_percentage_cgpa_ug='+obtained_percentage_cgpa_ug_val+'&yr_passing_ug='+yr_passing_ug_val+'&register_no_ug='+register_no_ug_val+'&schooling_id_tenth='+schooling_id_tenth_val+'&schooling_id_twelfth='+schooling_id_twelfth_val+'&twelfth_highest_edu_val='+twelfth_highest_edu_val+'&tenth_highest_edu_val='+tenth_highest_edu_val+'&program='+grad_id_hidden_val+'&digilocker_id='+digilocker_id_val+'&'+csrfName+'='+csrfHash;
	                    $.ajax({
	                				type: 'POST',
	                				url: url,
	                				data: user_data+'&currentIndex='+currentIndex,
						...
						register_no_ug.validate();
	                  	tenth_highest_edu.validate();
	                  	twelfth_highest_edu.validate();	
	                  	ug_highest_edu.validate();                 											
	                    return false;
	                  }
				}
					//end step5 academics
				...
				if(currentIndex == 6) {
		            ...
		            if(applicant_name.isValid() && parent_name.isValid() && date_dec.isValid())
		            {
        ...
        { 
			...
			if(currentIndex == parseInt(total_wizard_Step - 1)){
				...
				if(mode_edit_url !='')
		        {
		         enable_preview_btn(currentIndex,'<?php echo base_url();?>hspg_diploma_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
		        }else{
				enable_preview_btn(currentIndex,'<?php echo base_url();?>hspg_diploma_preview');
		        }
        ...
        onFinished: function (event, currentIndex) {            
        	 ...
        	 var other_document1=$('#other_document1').parsley();
        	 var other_document2=$('#other_document2').parsley();
            ...
            if(photograph.isValid() && signature.isValid() && other_document1.isValid() && other_document2.isValid())
            {
			$.ajax({
				type: 'POST',
				url: url,
            	...
            	photograph.validate();
            	signature.validate();						
            	other_document1.validate();
            	other_document2.validate();
            }
	...
	$("#hspg_diploma_form").steps(settings);
  //setsave exit attr	
    $(document).on("click", "#save_btn" , function() {
    	 $("#hspg_diploma_form").attr('isexit',1);
         $("#hspg_diploma_form").steps('next');
    });
    
    $('.actions a').click(function(){        	 
      $("#hspg_diploma_form").attr('isexit','');
    }); 
//end set attr
   //to remove links from previous tabs a
    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0) && ($mode_edit == '')) { ?>
     $( ".steps li" ).each(function( index ) { 
      if(index<6){       	 
   	  $("#hspg_diploma_form-t-"+index).removeAttr("href");
     }			   
   	});
    <?php } ?>
    light_box_init();
		...
		getSelect2('pd_social_status','<?php echo base_url("get_social_status"); ?>?is_lookup_master=1'+url_edit,'Select Social Status'+url_edit, formatRepoCommon,no_social_status_msg, false, formatRepoSelection);
		...
		getSelect2('current_qualification_status','<?php echo base_url("qualification_status"); ?>?grad_id='+grad_id+'&sort_by=name&sort_order=asc'+url_edit,'Select Current Qualification Status', formatRepoCommon,no_current_qualification_status, false, formatRepoSelection);		
		...
		getSelect2('tenth_highest_edu','<?php echo base_url("high_education"); ?>?type='+sslcType+'&sort_by=name&sort_order=asc'+url_edit,'Select', formatRepoCommon,no_resident_status_msg, false, formatRepoSelection);
		getSelect2('ug_highest_edu','<?php echo base_url("high_education"); ?>?type='+ugType+'&sort_by=name&sort_order=asc'+url_edit,'Select', formatRepoCommon,no_resident_status_msg, false, formatRepoSelection);
		...
		getSelect2('twelfth_highest_edu','<?php echo base_url("high_education"); ?>?type='+hscType+'&sort_by=name&sort_order=asc'+url_edit,'Select', formatRepoCommon,no_resident_status_msg, false, formatRepoSelection);
       ...
       ?>
		...
		var dbboardtth_val = '<?php echo $board_name_twelfth; ?>';
		 ...
		 if(dbboardtth_id){
			...
			<?php
			$high_edu=$this->config->item('high_edu');
			?>

	    var tenth_highest_edu = "tenth_highest_edu";
		var dbtenth_highest_edu = '<?php echo $high_edu_tenth; ?>';
		var dbtenth_highest_edu_val = '<?php echo $high_edu[SCHOOLING_ID_TENTH][$high_edu_tenth];?>';
		 if(dbtenth_highest_edu){
			
			setTimeout(function(){ select2Set(tenth_highest_edu,dbtenth_highest_edu,dbtenth_highest_edu_val);
			}, 1);
		}

		var twelth_highest_edu = "twelfth_highest_edu";
		var dbtwelth_highest_edu = '<?php echo $high_edu_twelfth; ?>';
		var dbtwelth_highest_edu_val = '<?php echo $high_edu[SCHOOLING_ID_TWELFTH][$high_edu_twelfth];?>';
		
		 if(dbtwelth_highest_edu){
			setTimeout(function(){ select2Set(twelth_highest_edu,dbtwelth_highest_edu,dbtwelth_highest_edu_val);
			}, 1);
		}
       ...
       //ug univ
		...
		if(dbuniv_ug_id){
 		...
 		var ug_highest_edu = "ug_highest_edu";
		var dbug_highest_edu = '<?php echo $applicant_graduations_list[0]['graduation_type_id']; ?>';
		var dbug_highest_edu_val = '<?php echo $high_edu[GRADUATION_UG_ID][$applicant_graduations_list[0]['graduation_type_id']];?>';
		
		 if(dbug_highest_edu){
			setTimeout(function(){ select2Set(ug_highest_edu,dbug_highest_edu,dbug_highest_edu_val);
			}, 1);
		}	
	  ...
	  $('#pd_program').on('change',function() {
			...
			if(program=="" || program=='null' || program==null){
				$('#campus_div').hide();
			}else{
			$('#campus_div').show();
			...
			getSelect2('campus_pref_1','<?php echo base_url("course_preference"); ?>?is_lookup_master=1&is_campus=1&grade_id='+grad_id+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select Campus', formatRepoCommon,no_campus_msg, false, formatRepoSelection);
			...
			//set academic section data based on program level
			if(program==diploma){
				 $("#qualification_status").hide();
				 $('#appering').attr('data-parsley-required', 'false');
				 $('#completed').attr('data-parsley-required', 'false');				 
               ...
               if(program==pg || program==pg_diploma){
              	     $("#qualification_status").show();
    				 $('#appering').attr('data-parsley-required', 'true');
    				 $('#completed').attr('data-parsley-required', 'true');				 
               ...
               if(program==ug ||program==pg || program==pg_diploma){
            	 $("#qualification_status").show();
    			 $('#appering').attr('data-parsley-required', 'true');
    			 $('#completed').attr('data-parsley-required', 'true');				 
                ...
                $('.ugreq').hide();
  	            ...
  	            $('.ugtable select').attr('data-parsley-required', 'false');
      	          if(program==pg || program==pg_diploma){
      				...
      				$('.ugtable input').attr('data-parsley-required', 'true');
      	            $('.ugtable select').attr('data-parsley-required', 'true');
      	          <?php if($qual_status_id==HSPG_DIP_EDU_PURSUING){ ?>
          	        $("#user_marking_scheme_ug").attr('data-parsley-required','false');
                    $("#obtained_percentage_cgpa_ug").attr('data-parsley-required','false');
                   <?php } ?>
      	          }
      	        if(program==ug){
      	        	$("#user_marking_scheme_ug").attr('data-parsley-required','false');
                    $("#obtained_percentage_cgpa_ug").attr('data-parsley-required','false');
                    <?php if($qual_status_id==HSPG_DIP_EDU_HSC_PURSUING){ ?>
          	        $("#academic_marking_scheme_twelfth").attr('data-parsley-required','false');
                    $("#academic_obtain_cgpa_twelfth").attr('data-parsley-required','false');
                    <?php } ?>
      	        }
		...
		<?php			
		 if(!empty($qual_status_id)){
		     if($grad_id==PG_GRAD ||$grad_id==PG_DIPLOMA_GRAD){
			     if($qual_status_id == HSPG_DIP_EDU_PASSED) {
			         ?>
			         
		            	$("#completed").trigger('click');
		            	<?php
			     }
			      if($qual_status_id == HSPG_DIP_EDU_PURSUING)
			     {
			         ?>
		     ...
		     if($grad_id==UG_GRAD){
		         if($qual_status_id == HSPG_DIP_EDU_HSC_PASSED) {
		             ?>
			         
		            	$("#completed").trigger('click');
		            	<?php
			     }
			     if($qual_status_id == HSPG_DIP_EDU_HSC_PURSUING)
			     {
			         ?>
	            	$("#appering").trigger('click');
	            	<?php
		...
		$('#academic_stream').on('change',function() {
		 ...
		 $('#course_pref_1').on('change',function() {						 
			 course_pref_change('course_pref_1','course_pref_2','course_pref_3',1);
         });

		 $('#course_pref_2').on('change',function() {
			 $("#custom-field-course-error").html('');					 
			 course_pref_change('course_pref_2','course_pref_3','course_pref_1',2);
		 });
		 ...
		 $('#course_pref_3').on('change',function() {
			 ...
			 {
                $("#custom-field-course-error").html("<?php echo REQD_SELECT_CAMPUS_PREFERENCE2_MSG;?>");
                $("#course_pref_3").val('');               
			 }else{				 
				 $("#custom-field-course-error").html('');
			 }	
		...
		$("#academic_yr_passing").datepicker( {
			format:"yyyy" , autoclose: !0, minViewMode: 'years', endDate: moment().format('dd-mm-yyyy'),startDate: '-40y',endDate: '+0d' 
                ...
                if(Yr!="" && Yr!="YYYY"){                
             	...
             	$("#academic_yr_passing_twelfth").attr("min",setYr);
             	$("#yr_passing_ug").attr("min",setYr);
             	<?php if($grad_id==DIPLOMA_GRAD) { ?>
             	if($('#academic_yr_passing_twelfth').val()==''){
             	 $("#academic_yr_passing_twelfth").removeAttr("min");
             	}
                $("#yr_passing_ug").removeAttr("min");
             	<?php } ?>
		...
		// This Function for Payment option selection
			...
			setTimeout(function() { 
				if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
					...
					if(mode_edit_url !='')
			        {
			         enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>hspg_diploma_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
			        }else{
					enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>hspg_diploma_preview');
			        }
				}
			},1);
...
function course_pref_change(currentcourse,reset_course,other_course,choice) {
    var current_course_val=$("#"+currentcourse).val();
    var other_course_val=$("#"+other_course).val();
    var no_course_msg = "Sorry, Course not available.";
	
    //for reset1 
	var notCourseList = [];
	if(current_course_val !== null  && current_course_val !== ""){
		notCourseList.push(current_course_val);
    }
	if(other_course_val !== null && other_course_val !== ""){
		notCourseList.push(other_course_val);
    }
    if(notCourseList.length>0);
    {
		var notCourseListVal=notCourseList.join(",");
		setEmptyOnChangeSelect2(reset_course);
		if ($('#'+reset_course).data('select2')) {
			$('#'+reset_course).select2('destroy');
		}				
		$('#'+reset_course).html(''); 
		
		getSelect2(reset_course,'<?php echo base_url("course_preference"); ?>?is_lookup_master=1&ecourse='+notCourseListVal+'&is_course=1&grade_id='+grad_id+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select', formatRepoCommon,no_course_msg, false, formatRepoSelection); 
		if(choice==1){
			setEmptyOnChangeSelect2(other_course);
			if ($('#'+other_course).data('select2')) {
				$('#'+other_course).select2('destroy');
			}				
			$('#'+other_course).html(''); 				
			getSelect2(other_course,'<?php echo base_url("course_preference"); ?>?is_lookup_master=1&ecourse='+notCourseListVal+'&is_course=1&grade_id='+grad_id+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select', formatRepoCommon,no_course_msg, false, formatRepoSelection); 
		}
	}
...
function enable_course_by_campus(campus,course,course_div,course_placeholder,grad_id,choice) {
	$('#'+campus).on('change',function() {
		...
		if(campus_val=="" || campus_val=='null' || campus_val==null){
		    ...
		    $("#course_div").show();
    		var no_course_msg = "Sorry, Course not available.";		
    		getSelect2('course_pref_1','<?php echo base_url("course_preference"); ?>?is_lookup_master=1&is_course=1&grade_id='+grad_id+'&campus_id='+campus_val+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select', formatRepoCommon,no_course_msg, false, formatRepoSelection);
    		getSelect2('course_pref_2','<?php echo base_url("course_preference"); ?>?is_lookup_master=1&is_course=1&grade_id='+grad_id+'&campus_id='+campus_val+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select', formatRepoCommon,no_course_msg, false, formatRepoSelection);
    		getSelect2('course_pref_3','<?php echo base_url("course_preference"); ?>?is_lookup_master=1&is_course=1&grade_id='+grad_id+'&campus_id='+campus_val+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select', formatRepoCommon,no_course_msg, false, formatRepoSelection);
    		}
		});
}
