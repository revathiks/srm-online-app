 ...
 *
 */
class Apg extends API_Controller {
	...
	public function basic_details_apg_post(){
		$app_form_id_pgdplife = APP_FORM_ID_PGDPLIFE;
		...
		$_POST['active'] = TRUE;
		$_POST['application_status_id'] = ($appln_status==APPLICATION_IN_PROGRESS || $appln_status=='')?APPLICATION_IN_PROGRESS:APPLICATION_IN_COMPLETED;
		$_POST['grad_id'] = PG_DIPLOMA_GRAD;
		...
		$applicant_appln_det_res = $this->_get_appln_form_id($applicant_personal_det_id,$app_form_id_pgdplife);
		...
		if($qualifying_exam_from_india) {
			if($qualifying_exam_from_india['status'] == 'error') {
				...
				// Update Wizard ID
				$response['forn_w_wizard_id'] = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_pgdplife , FORM_WIZARD_BASIC_ID);				
	...
	public function personal_detail_apg_post(){
		$app_form_id_pgdplife = APP_FORM_ID_PGDPLIFE;
		$table_schema = SCHEMA_ADMISSION;
		...
		// Update Wizard ID
		$spec_pref_res['forn_w_wizard_id'] = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_pgdplife , FORM_WIZARD_PERSONAL_ID);
	...
	public function db_func_call_apg_campus_get() {
		$get_prog_id = $this->input->get('prog_id');
		$form_id = APP_FORM_ID_PGDPLIFE;
		$fn_get_app_course_detail = FN_GET_CHOICE_DROPDOWN;
		$table_schema = SCHEMA_ADMISSION;
	...
	public function apg_parent_detail_post() {
    	$app_form_id = APP_FORM_ID_PGDPLIFE;
		...
		// Wizard ID Update
		$parent_res['wizard_id_update'] = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_PGDPLIFE , FORM_WIZARD_PARENT_ID);
	...
	public function apg_academic_detail_post() {
    	$app_form_id_pgdplife = APP_FORM_ID_PGDPLIFE;
		...
		$grad_columns = "insti_name,univ_id,mark_scheme_id,mark_percent,completion_year,registration_no,graduation_type_id,is_curr_qualify,applicant_appln_id,result_declared,applicant_personal_det_id,active,other_university_name";
		...
		if($academic_pg_graduation_inst && $curr_edu_quali_status==2){
		...
		$columns = 'applicant_personal_det_id,institute_name,board_id,marking_scheme_id,mark_percentage,completion_year,registration_no,schooling_id,active,result_declared,name_as_in_marksheet,applicant_appln_id,other_board_name,is_curr_qualify';
		...
		// Wizard ID Update
		$parent_res['wizard_id_update'] = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_PGDPLIFE , FORM_WIZARD_ACADEMIC_ID);
	...
	public function declaration_apg_post(){
		...
		if($user_data['currentIndex']==6){
			...
			$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_PGDPLIFE , FORM_WIZARD_UPLOAD_DECLARATION_ID);
