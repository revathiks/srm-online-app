...
//Campus Prefer
//print_r($applicant_campus_details_list);die;
...
<script>
  var app_form_id = '<?php echo $app_form_id; ?>';
  ...
  var otheroccupation='<?php echo OTHER_OCCUPATION;?>';
    ...
    <?php if($mode_edit) { ?>
      ...
      var url = "<?php echo base_url(); ?>law/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
 ...
 csrfHash = '<?php echo $this->security->get_csrf_hash(); ?>';
    ...
    <?php
      ...
      setTimeout(function() { 
        if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
        	$("#save_exit").remove();
        	//enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>law_form_preview');
          if(mode_edit_url != '')
          {
            enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>law_form_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
          }else{
            enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>law_form_preview');
          }
        }
    ...
    var settings = {
      ...
      if(currentIndex < newIndex) {
        ...
        if(currentIndex === 0) {
          ...
          if(graduation_india.isValid() && qualifyingexamfromindia.isValid()) {
        ...
        if(currentIndex === 1) {
          ...
          var course_pref= $('#course_pref').parsley();                  
          ...
          if(title_id.isValid() && first_name.isValid() && last_name.isValid() && mtech_mobile_no.isValid() && dob.isValid() && gender_id.isValid() && blood_group_id.isValid() && email_id.isValid() && course_pref.isValid() && campus_pref_1.isValid() && nationality_id.isValid() && middle_name.isValid() && pd_alt_email.isValid() && pd_diffrently_abled.isValid() && social_status.isValid()) {
            var program_level_val= $('#program_level').val();
            var campus_pref_1=$('#campus_pref_1').val();
            var course_pref =$('#course_pref').val();
            var pgm_level_id = $('#pgm_level_id').val();
            var campus_prefer_id_1 = $('#campus_prefer_id_1').val();
            var course_prefer_id = $('#course_prefer_id').val();
          ...
          var user_data = 'campus_pref='+campus_pref_1+'&grad_id='+program_level_val+'&course_pref='+course_pref+'&title_id='+title_id_val+'&first_name='+first_name_val+'&last_name='+last_name_val+'&middle_name='+middle_name_val+'&email_id='+email_id_val+'&pd_dob='+dob_val+'&gender_id='+gender_id_val+'&second_email_id='+second_email_id_val+'&blood_grp_id='+blood_grp_id_val+'&religion_id='+religion_id_val+'&mothertongue_id='+mothertongue_id_val+'&medofinst='+medofinst_val+'&prefer_hostel='+prefer_hostel_val+'&diff_abled='+diff_abled_val+'&advertisement_source_id='+advertisement_source_id_val+'&nationality_id='+nationality_id_val+'&mobile_no='+mobile_no_val+'&phone_no_code='+phone_no_code_val+'&campus_prefer_id='+campus_prefer_id_1+'&course_prefer_id='+course_prefer_id+'&campus_choice_no=1'+'&course_choice_no=1&social_status_id='+pd_social_status_val+'&'+csrfName+'='+csrfHash;
          var moveNxt=0;
          $.ajax({
            type: 'POST',
            url: url,
        ...
        if(currentIndex === 3) {
          ...
          if(country.isValid() && state.isValid() && district.isValid() && city.isValid() && address1.isValid() && pincode.isValid()) {
                ...
                success: function(returndata) {
                if(returndata) {                  
                var address_status = returndata.address_response.status;
                if(address_status === true || address_status === 'true')
                {
                  if(isexit==1){
                        window.location.href = save_exit_redirect_url;
                        return false; } else {
            ...
            var academic_stream_val = $('#academic_stream').val();
      ...
      if(currentIndex == parseInt(total_wizard_Step - 1)){
    	  $("#save_exit").remove();
    	  //enable_preview_btn(currentIndex,'<?php echo base_url();?>law_form_preview');
        if(mode_edit_url !='')
        {
          enable_preview_btn(currentIndex,'<?php echo base_url();?>law_form_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
        } else {
          enable_preview_btn(currentIndex,'<?php echo base_url();?>law_form_preview');
        }
      }else{
    ...
    $("#law_form").steps(settings);
  //setsave exit attr	
    $(document).on("click", "#save_btn" , function() {
    	 $("#law_form").attr('isexit',1);
         $("#law_form").steps('next');
    });
    
    $('.actions a').click(function(){        	 
      $("#law_form").attr('isexit','');
    }); 
//end set attr
  //to remove links from previous tabs a
    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0) 
      && ($mode_edit == '')) { ?>
     $( ".steps li" ).each(function( index ) { 
      if(index<6){         
      $("#law_form-t-"+index).removeAttr("href");
     }         
    });
    <?php } ?>
        $(document).ready(function () {
      ...
      var no_institute_university_msg = "<?php echo NO_UNIVERSITY_MSG;?>";
      ...
      var no_current_qualification_status = "Sorry, Current Qualification not available.";
      ...
      getSelect2('program_level','<?php echo base_url("get_choice_dropdown"); ?>?is_lookup_master=1&is_program=1&is_distinct=1'+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Choose Program Level', formatRepoCommon,no_course_msg, false, formatRepoSelection);
     ...
     /* getSelect2('course_pref_3','<?php echo base_url("get_choice_dropdown"); ?>?is_lookup_master=1&is_course=1&is_distinct=1&grad_id='+const_grad_id+'&deg_id='+const_deg_id+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc','Choose Course Perferences 1', formatRepoCommon,no_course_msg, false, formatRepoSelection);*/
      ...
      course_pref_change('program_level','campus_pref_1','main_div_camp_pref_1','Choose Campus Perferences');

      campus_pref_change('campus_pref_1','course_pref','main_div_course_pref','Choose Course Perferences 2');
      ...
      getSelect2('current_qualification_status','<?php echo base_url("law_edu_status"); ?>?sort_by=name&sort_order=asc'+url_edit,'Select Current Qualification Status', formatRepoCommon,no_current_qualification_status, false, formatRepoSelection);
     ...
     getSelect2('institute_university_1','<?php echo base_url("get_institute_university"); ?>?sort_by=name&sort_order=asc'+url_edit,'Choose Institute/University', formatRepoCommon,no_institute_university_msg, false, formatRepoSelection);
      ...
      getSelect2('user_marking_scheme_3','<?php echo base_url("get_user_marking_scheme"); ?>?is_lookup_master=1'+url_edit,'Choose Marking Scheme', formatRepoCommon,no_user_marking_scheme_msg, false, formatRepoSelection);
            ...
            /*getSelect2('competitive_exam_0','<?php echo base_url("get_competitive_exams"); ?>?sort_by=name&sort_order=asc','Choose Competitive Exam', formatRepoCommon,no_competitive_exam_msg, false, formatRepoSelection);
            getSelect2('competitive_exam_1','<?php echo base_url("get_competitive_exams"); ?>?sort_by=name&sort_order=asc','Choose Competitive Exam', formatRepoCommon,no_competitive_exam_msg, false, formatRepoSelection);
            getSelect2('competitive_exam_2','<?php echo base_url("get_competitive_exams"); ?>?sort_by=name&sort_order=asc','Choose Competitive Exam', formatRepoCommon,no_competitive_exam_msg, false, formatRepoSelection);
            */

             getSelect2('competitive_exam_0','<?php echo base_url("competitive_exam_list"); ?>?form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Select Competitive Examination', formatRepoCommon,no_competitive_exam_msg, false, formatRepoSelection);
    ...
    $('#district').on('change',function() {
      ...
      // $("#pd_title").select2();
            ...
            $("#resident_info").change(function () {
                if ($("#resident_info").val() == 'non-resident-indian' || $("#resident_info").val() == 'foreign') {
      ...
      var db_grad_id = '<?php echo $grad_id;?>';
      var db_grad_name = '<?php echo $grad_name;?>';
      if(db_grad_id){
        setTimeout(function(){ select2Set(program_level,db_grad_id,db_grad_name);
        }, 1);
      }
      ...
      var dbstreamtwfth_id = '<?php echo $applicant_school_stream_id[1]; ?>';
      var dbcstreamtwfth_val = '<?php echo $applicant_school_stream_name[1]; ?>';
      //alert(dbcountry_val);
       if(dbstreamtwfth_id){
 ...
 onkeyValidationCEXAM('competitive_exam_2','rollno_2','score_obtained_2','max_score_2','year_appered_2','overall_rank_2','qualify_2');
  ...
  // }).swidget();
    ...
    function course_pref_change(course,campus,campus_div,campus_placeholder) {
      ...
      getSelect2(campus,'<?php echo base_url("get_choice_dropdown"); ?>?form_id='+app_form_id+'&is_lookup_master=1&is_campus=1&is_distinct=1'+'&grad_id='+const_grad_id+'&sort_by=id&sort_order=asc'+url_edit,campus_placeholder, formatRepoCommon,no_campus_prefer_msg, false, formatRepoSelection);
    });
    }
    ...
    function declaration_func(currentIndex) {
      var photograph = $('#photograph').parsley();
       ...
       // var tenth_marksheet = $('#tenth_marksheet').parsley();
        ...
        success: function(returndata) {
          if(returndata.status == 'true' || returndata.status == true) {
            setTimeout(function() { window.location.href = redirect_thank+url_edit; 
          }, 100);
            $("#formerr").hide();
                moveNxt=1;
          }
...
function campus_pref_change(campus,course,course_div,course_placeholder)
    {
      ...
      getSelect2(course,'<?php echo base_url("get_choice_dropdown"); ?>?is_lookup_master=1&is_course=1&is_distinct=1&grad_id='+const_grad_id+'&form_id='+app_form_id+'&campus_id='+campus_val+'&sort_by=name&sort_order=asc'+url_edit,course_placeholder, formatRepoCommon,no_course_msg, false, formatRepoSelection);
