 ...
 *
 */
class Phd extends API_Controller {
	...
	public function wizard_api_post(){
		...
		if($user_data['currentIndex']==5){
			...
			if($check_applicant_pid['data'][0]['applicant_appln_id']==$applicant_appln_id){
				...
				$put_arrays['appln_form_id'] = APP_FORM_ID_PHD;
	...
	public function phd_acdemic_dtl_post()
	{
			$app_form_id_phd = APP_FORM_ID_PHD;
    ...
    public function phd_final_step_post() {
		$table_schema = SCHEMA_ADMISSION;
		$tableaad = APPLICANT_APPLN;
		$app_form_id_phd = APP_FORM_ID_PHD;
		$form_wizard_upload_id = FORM_WIZARD_UPLOAD_ID;
		...
		$applicant_appln_columns = 'updated_by';
		$_SERVER["REQUEST_METHOD"] = "POST";
		$_POST['updated_by'] = $updated_by;
		$function_name = $this->get_function_name(__FUNCTION__);
		$update = $this->_insert_update_applicant_tables($function_name, array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id), $table_schema, $tableaad, $applicant_appln_columns, 2);		
