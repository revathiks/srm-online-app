 ...
 *
 */
class Distance_jan extends API_Controller {
    ...
    public function wizard_api_dist_edu_post(){
		...
		if($user_data['currentIndex']==0){
			...
			*/
			...
			$update = $this->common->common_update ( $tablepd ,'' , $put_arraypd, $wherepd);$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_DE_JAN , FORM_WIZARD_BASIC_ID);			
		...
		if($user_data['currentIndex']==1){
			...
			if($check_applicant_pid_in_appln['data'][0]['applicant_appln_id']==$applicant_appln_det_id){
				...
				$put_arrays['appln_form_id'] = APP_FORM_ID_DE_JAN;
				...
				$_POST['appln_form_id']= APP_FORM_ID_DE_JAN;
			...
			// Update Wizard ID
			$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_DE_JAN , FORM_WIZARD_PERSONAL_ID);			
			$this->response (['status' => 'true' ,'message' => RECORD_UPDATE_SUCCESSFULLY,] , API_Controller::HTTP_OK);
		}	
	}
	
	public function wizard_api_parent_edu_post(){
		$user_data = $this->post();
		$created_by = $this->post('created_by');
		$updated_by = $this->post('updated_by');		
		$applicant_id = $user_data['applicant_id'];
		...
		// Update Wizard ID
		$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_DE_JAN , FORM_WIZARD_PARENT_ID);		
		$this->output->set_output(json_encode(['status' => '3', 'user_data' => $user_data]));  
	}
	
	public function dis_edu_academic_post(){
		$user_data = $this->post();
		$created_by = $this->post('created_by');
		$updated_by = $this->post('updated_by');
		$table_schema = SCHEMA_ADMISSION;
		...
		$applicant_other_details_columns ='applicant_personal_det_id,is_work_experience,active,created_by,updated_by';
		
		$other_res = $this->_insert_update_applicant_tables($function_name, $applicant_id, $table_schema, $applicant_other_details_table, $applicant_other_details_columns); */
		...
		$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_DE_JAN , FORM_WIZARD_ACADEMIC_ID);
	...
	public function declaration_dde_post(){
		...
		$put_arrayaad['application_status_id'] = $app_status;
		$put_arrayaad['updated_by']= $updated_by;
		$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_DE_JAN , FORM_WIZARD_UPLOAD_DECLARATION_ID);
		$update = $this->common->common_update ( $table_appln ,'' , $put_arrayaad, $whereaad);
