 ...
 *
 */
class Tamil extends API_Controller {
	...
	public function basic_details_tamil_post(){
		
		$app_form_id_tamil  		  = APP_FORM_ID_TAMIL_PERAYAM;
		...
		// Columns
		$applicant_appln_columns   = 'active';

		$applicant_course_prefer_columns = 'applicant_personal_det_id,course_id,course_name,choice_no,active,applicant_appln_id,deg_id';
		...
		$n_course_prefer_id = $this->post('course_prefer_id',true);

		// get applicant form pk id
		$applicant_appln_det_res = $this->_get_appln_form_id($applicant_personal_det_id,$app_form_id_tamil);
		$applicant_appln_id      = $applicant_appln_det_res['id'];
		$deg_id = $this->post('deg_id',true);
		$course_dl = $this->post('course_pref',true);
		$course_pref = explode(',', $course_dl);
		if($deg_id == 43){
			$grad_mode_id = 2;
		}else { $grad_mode_id = 1; }
		...
		$applicant_course_prefer_res = $this->_get_prog_id_by_preference_dtl(false, $deg_id, $app_form_id_tamil, $course_pref,false,false,$grad_mode_id);
		...
		$_POST['applicant_appln_id'] 	= $applicant_appln_id;
		$_POST['course_id'] 	= $applicant_course_prefer_res[0];
		...
		$this->form_validation->set_rules('deg_id', 'Degree id','trim|required');
		$this->form_validation->set_rules('course_pref', 'Course id','trim|required');
		...
		if($qualifying_exam_from_india) {
			if($qualifying_exam_from_india['status'] == 'error') {
				...
				// Update Wizard ID
				$response['forn_w_wizard_id'] = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_tamil , FORM_WIZARD_BASIC_ID);				
	...
	public function personal_detail_tamil_post(){
		...
		// Columns
		//$applicant_course_det_columns = 'course_id,choice_no,applicant_personal_det_id,created_by,updated_by,active,applicant_appln_id';
		
		//$applicant_campus_det_columns = 'choice_no,campus_id,applicant_personal_det_id,created_by,updated_by,active,applicant_appln_id';
		...
		*/
		...
		// Update Wizard ID
		$spec_pref_res['forn_w_wizard_id'] = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_tamil , FORM_WIZARD_PERSONAL_ID);
		
		$this->response ($spec_pref_res , API_Controller::HTTP_OK );
	}		
	
	// Advanced PG Diploma Life Science Select Course and Campus Preference Dropdown
	/* public function db_func_call_apg_course_get() {
		$get_prog_id = $this->input->get('prog_id');
		$is_course =  $this->get('is_course',true);
		$form_id = APP_FORM_ID_TAMIL_PERAYAM;
	...
	*/
	//db_func_call_apg_campus_get 
	public function db_func_call_tamil_course_get() {
		$get_prog_id = $this->input->get('degree_id'); 
		if ($get_prog_id==56) {
		$get_degree_type_id = 1;
		} else if ($get_prog_id==43){
		$get_degree_type_id = 2;	
		}
		$form_id = APP_FORM_ID_TAMIL_PERAYAM;
		$fn_get_app_course_detail = FN_GET_CHOICE_DROPDOWN;
		$table_schema = SCHEMA_ADMISSION;
		...
		$array3=!empty($array3) ? $array3 : 'null';
		//$array4=!empty($array4) ? $array4 : 'null';
		//$array5=!empty($array5) ? $array5 : 'null';
		$form_id=!empty($form_id) ? $form_id : 'null';
		
		$arguments=array($array1,$array2,$array3,$get_prog_id,$form_id,$get_degree_type_id);
		//$arguments=array(null,null,null,56,28,1);
		//(null,null,null,56,28,1)		
		$result = $this->db_distinct_func_call($table_schema, $fn_get_app_course_detail, $arguments,$column);
	}	
	
	
	public function tamil_parent_detail_post() {
    	
		$app_form_id 					= APP_FORM_ID_TAMIL_PERAYAM;
		...
		// Wizard ID Update
		$parent_res['wizard_id_update'] = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_TAMIL_PERAYAM , FORM_WIZARD_PARENT_ID);
	...
	public function tamil_academic_detail_post() {
		...
		if($is_graduation){
			$graduation_check = $this->check_exist_user($graduation_table, $table_schema,$grad_columns,  array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id,'graduation_type_id'=>UG_GRAD),1,'applicant_grad_det_id');
			...
			if($graduation_check['data'][0]['applicant_appln_id']==$applicant_appln_id){
				...
				$wheregraduation['graduation_type_id'] = UG_GRAD;
				...
				$put_arraygraduation['graduation_type_id'] = UG_GRAD;
				$put_arraygraduation['applicant_appln_id']=$applicant_appln_id;
				$put_arraygraduation['updated_by']=$applicant_appln_id;
				...
				$parent_res['is_graduation'] = array("status"=>"Record updated successfully","type"=>"graduation detail");
			}else{
				...
				$_POST['graduation_type_id'] = UG_GRAD;
				$_POST['applicant_appln_id']=$applicant_appln_id;
				$_POST['created_by']=$applicant_appln_id;
				...
				$parent_res['is_graduation'] = array("status"=>"Record added successfully","type"=>"graduation detail");
			}
		}
			
		if($academic_pg_graduation_inst){
			$pg_graduation_check = $this->check_exist_user($graduation_table, $table_schema,$grad_columns,  array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id,'graduation_type_id'=>PG_GRAD),1,'applicant_grad_det_id');
							
			if($pg_graduation_check['data'][0]['applicant_appln_id']==$applicant_appln_id){
				$wheregraduation['applicant_personal_det_id'] = $applicant_personal_det_id;
				$wheregraduation['applicant_appln_id'] = $applicant_appln_id;
				$wheregraduation['graduation_type_id'] = PG_GRAD;
				$put_arraygraduation['insti_name'] = $academic_pg_graduation_inst;
				$put_arraygraduation['univ_id'] = $academic_pg_graduation_board;
				$put_arraygraduation['mark_scheme_id'] = $academic_marking_scheme_pg_graduation;
				$put_arraygraduation['mark_percent'] = $academic_obtain_cgpa_pg_graduation;
				$put_arraygraduation['completion_year'] = $academic_yr_passing_pg_graduation;
				$put_arraygraduation['registration_no'] = $academic_reg_no_pg_graduation;
				$put_arraygraduation['graduation_type_id'] = PG_GRAD;
				$put_arraygraduation['applicant_appln_id']=$applicant_appln_id;
				$put_arraygraduation['updated_by']=$applicant_appln_id;
				$put_arraygraduation['is_curr_qualify'] = $is_curr_qualify_pg;
				$put_arraygraduation['result_declared']=$result_declared_pg;
				$put_arraygraduation['applicant_personal_det_id']=$applicant_personal_det_id;
				if($other_board_pg_graduation){
					$put_arraygraduation['other_university_name'] = $other_board_pg_graduation;
				}				
				$this->common->common_update ($table_schema.'.'.$graduation_table ,'' , $put_arraygraduation, $wheregraduation);
				$parent_res['is_pg_graduation'] = array("status"=>"Record updated successfully","type"=>"pg graduation detail");
			}else{
				$_POST['insti_name'] = $academic_pg_graduation_inst;
				$_POST['univ_id'] = $academic_pg_graduation_board;
				$_POST['mark_scheme_id'] = $academic_marking_scheme_pg_graduation;
				$_POST['mark_percent'] = $academic_obtain_cgpa_pg_graduation;
				$_POST['completion_year'] = $academic_yr_passing_pg_graduation;
				$_POST['registration_no'] = $academic_reg_no_pg_graduation;
				$_POST['graduation_type_id'] = PG_GRAD;
				$_POST['applicant_appln_id']=$applicant_appln_id;
				$_POST['created_by']=$applicant_appln_id;
				$_POST['is_curr_qualify'] = $is_curr_qualify_pg;
				$_POST['result_declared']=$result_declared_pg;
				$_POST['active']=true;
				$_POST['applicant_personal_det_id']=$applicant_personal_det_id;
				if($other_board_pg_graduation){
					$_POST['other_university_name'] = $other_board_pg_graduation;
				}
				
				$function_name = $this->get_function_name ( __FUNCTION__ );      
				$this->_common_insert ( $graduation_table , $table_prefix , $function_name , $grad_columns , $table_schema);
				$parent_res['is_pg_graduation'] = array("status"=>"Record added successfully","type"=>"pg graduation detail");
			}
		}else{
			//($add_guardian_details!="true"){
				$whereparentgno = array();
				$whereparentgno['applicant_personal_det_id'] = $applicant_personal_det_id;
		...
		*/
		...
		$columns = 'applicant_personal_det_id,institute_name,board_id,marking_scheme_id,mark_percentage,completion_year,registration_no,schooling_id,active,created_by,updated_by,result_declared,name_as_in_marksheet,applicant_appln_id,other_board_name,is_curr_qualify';
		...
		if($tenth_scholling_det_id){
			...
			if($check_applicant_id_sd['data'][0]['applicant_appln_id']==$applicant_appln_id){
				...
				$put_arrayschool['updated_by']= $applicant_personal_det_id;
		...
		*/
		...
		// Wizard ID Update
		$parent_res['wizard_id_update'] = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_TAMIL_PERAYAM , FORM_WIZARD_ACADEMIC_ID);
	...
	public function declaration_tamil_post(){
		...
		if($user_data['currentIndex']==6){
			...
			$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_TAMIL_PERAYAM , FORM_WIZARD_UPLOAD_DECLARATION_ID);
