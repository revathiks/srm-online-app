...
class Shug extends FrontendController
{
	//
    public $CI;
    ...
    public function shug_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) { 
        ...
        if ( $this->input->post () ){
           ...
           // $userdata = $this->session_userdata();
            ...
            if($user_data['currentIndex']==1){
                ...
                $url = $api_urls[ 'shug_personal_detail' ];
                $userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
                // Get Personal ApplicaNT ID
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            if($user_data['currentIndex']==2){
                $url = $api_urls[ 'shug_parent_detail' ];
                $userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
                // Get Personal ApplicaNT ID
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            if($user_data['currentIndex']==4){
                ...
                $user_data['curr_qualify_ug'] = ($cur_qual_completed >=4 )?'t':'f';;
                $user_data['cur_qual_completed'] = ($cur_qual_completed==1 || $cur_qual_completed==3 || $cur_qual_completed==5 )?'t':'f';
                ...
                //11th
                $user_data['cur_qual_completed_eleven'] =($cur_qual_completed > 1 )?'t':'f';
                $user_data['schooling_id_eleven'] = $user_data['schooling_id_eleven'];
                $user_data['institute_name_eleven'] = $user_data['academic_eleven_inst'];
                $user_data['board_eleven'] = $user_data['academic_eleven_board'];
                $user_data['marking_scheme_eleven'] = ($user_data['academic_eleven_msv']!='null')? $user_data['academic_eleven_msv'] :'';
                $user_data['cgpa_eleven'] = ($user_data['academic_eleven_cgpa']!='null') ? $user_data['academic_eleven_cgpa'] :'';
                $user_data['yr_of_passing_eleven'] = $user_data['academic_eleven_yrp'];
                $user_data['roll_no_eleven'] = $user_data['academic_eleven_regno'];
                
                
                if($user_data['other_eleven_board']==OTHER_BOARD){
                    $user_data['other_eleven_board'] = $user_data['other_eleven_board'];
                }
                //end 11th
            ...
            $wizard_dt = $this->_get_wizard_details(APP_FORM_ID_SHUG);
            ...
            $this->data['site_title'] = $this->data['page_title'] = 'Science and Humanities Application Form ';
            $this->template('applications/form_shug', $this->data, true);
        }
    }
    public function shug_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
        $this->is_exists_user_logged();
        /*CRM ADMIN Edit form start*/
        $this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
        $this->data['mode_edit'] = $mode;
        ...
        $this->data['user_data'] = $this->user_details_data['user_details']['data'][0];
        $this->data['personal_detail_list'] = $this->personal_detail_list($applicant_id);
        $this->data['site_title'] = $this->data['page_title'] = 'Science and Humanities Application Form Preview';
        /* applicant_address_details_list end */
        
        // Display page with the template function from REST_Controller
        $this->template('applications/shug_preview', $this->data, true);
    }
    public function shug_preference() {
        $api_urls = $this->config->item ( 'api_urls' );
        $url= $api_urls['shug_preference'];
