...
class Hspgee extends FrontendController
{
	//
    public $CI;
    ...
    public function hspg_ee_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) { 
        ...
        // Get Form ID
        $get_form_id = APP_FORM_ID_HSPG_EE;
        // Get Applicant ID
        ...
        if ( $this->input->post () ){
            ...
            $user_data['app_form_id']=APP_FORM_ID_HSPG_EE;
            $app_form_id=APP_FORM_ID_HSPG_EE;
           
            // Basic Details Start
            if($user_data['currentIndex']==0){
                // Basic Details API & Method
                $url = $api_urls[ 'hspg_ee_basic_detail' ];
                $userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
            ...
            if($user_data['currentIndex']==1){
                ...
                $url = $api_urls[ 'hspg_ee_personal_detail' ];
                $userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
                // Get Personal ApplicaNT ID
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            if($user_data['currentIndex']==2){
                $url = $api_urls[ 'hspg_ee_parent_detail' ];
                $userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
                // Get Personal ApplicaNT ID
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            {
                $api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'hspg_ee_addressdet' ];
                $method = 'POST';
                $userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            if($user_data['currentIndex']==4){
                $api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'hspg_ee_academicdet' ];
                $method = 'POST';
                
                $user_data['applicant_id'] = $applicant_id;
                $cur_qual_completed = ($user_data['current_education_qual_status']==HSPG_EE_EDU_PASSED)?'t':'f';
                $user_data['qual_status_id'] = $user_data['current_education_qual_status'];
                $program=$user_data['program'];
                $user_data['ug_completed'] = $cur_qual_completed;
                $user_data['name_as_in_marksheet'] = $user_data['candidate_name'];
                $user_data['current_education_qual_status'] = $cur_qual_completed;
                ...
                //pg
                $user_data['pg_completed'] =$cur_qual_completed;
                ...
                $user_data['marking_scheme_pg'] = ($user_data['marking_scheme_pg']!='null')? $user_data['marking_scheme_pg'] :NULL;
                $user_data['obtained_percentage_cgpa_pg'] = ($user_data['obtained_percentage_cgpa_pg']!='null') ? $user_data['obtained_percentage_cgpa_pg'] :NULL;
            ...
            if($user_data['currentIndex']==7){
                $url = $api_urls[ 'hspg_ee_declaration' ];
                $userdata = $this->user_details_data;
                $user_data['userdata'] = $userdata;
            ...
            $wizard_dt = $this->_get_wizard_details(APP_FORM_ID_HSPG_EE);
            ...
            $this->data['site_title'] = $this->data['page_title'] = 'Health Science PG (Entrance Exam)';
            $this->template('applications/form_hspg_ee', $this->data, true);
        }
    }
    public function hspg_ee_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
        $this->is_exists_user_logged();
        /*CRM ADMIN Edit form start*/
        $this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
        $this->data['mode_edit'] = $mode;
        ...
        $this->data['user_data'] = $this->user_details_data['user_details']['data'][0];
        $this->data['personal_detail_list'] = $this->personal_detail_list($applicant_id);
        $this->data['site_title'] = $this->data['page_title'] = 'Health Science PG (Entrance Exam) Preview';
        /* applicant_address_details_list end */
        
        // Display page with the template function from REST_Controller
        $this->template('applications/hspg_ee_preview', $this->data, true);
    }
