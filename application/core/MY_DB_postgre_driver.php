...
class MY_DB_postgre_driver extends CI_DB_postgre_driver
{
    ...
    */
    ...
    public function trail($status, $event, $table, $set = NULL, $previous_values = NULL)
    {
		...
		if($this->CI->config->item('track_file_log')==TRUE){
			//log_message('DEBUG', "==================================================", false);
			log_message('DEBUG', " EVENT : ".$event, false,'logs');
			log_message('DEBUG', " TABLE NAME : ".$table, false,'logs');
			log_message('DEBUG', " OLD VALUES : ".$old_value, false,'logs');
			log_message('DEBUG', " NEW VALUES : ".$new_value, false,'logs');
			log_message('DEBUG', " URL : ".$this->CI->uri->ruri_string(), false,'logs');
			log_message('DEBUG', " NAME : ".AUDIT_LOG_NAME, false,'logs');
			log_message('DEBUG', " IP ADDRESS : ".$this->CI->input->ip_address(), false,'logs');
			log_message('DEBUG', " USER AGENT : ".$this->CI->input->user_agent(), false,'logs');
			log_message('DEBUG', " CREATED AT : ".date('Y-m-d H:i:s'), false,'logs');
			//log_message('DEBUG', "==================================================", false);
			//exit;
		}		
		else{
			parent::insert(SCHEMA_PHP_DEV.'.user_audit_trails' ,
            [
                'user_id' => AUDIT_LOG_USERID,
                'event' => $event,
                'table_name' => $table,
