...
|
*/
$config[ 'rest_db_schema' ] = 'php_dev';
$ci = &get_instance ();
$db = $ci->load->database ();
// $ci->db->simple_query ( 'SET search_path TO configuration,crm_mobile' );
$ci->db->simple_query ( 'SET search_path TO '.$config[ 'rest_db_schema' ].',public' );
$ci->db->select ( array ( 'username' , 'password' ) );
$ci->db->from ( $config[ 'rest_db_schema' ].'.'.'api_logins' );
...
if ( $result ) {
    foreach ($result as $k => $v) {
        $results[ $v[ 'username' ] ] = $v[ 'password' ];
    }
}
$config[ 'rest_valid_logins' ] = $results;
...
| column name to match e.g. REST_key
|
*/
...
$config[ 'rest_db_log' ] = FALSE;

$config[ 'rest_file_log' ] = TRUE;
...
| table name to match e.g. REST_logs
|
*/
...
| table name to match e.g. REST_access
|
*/
...
| table name to match e.g. REST_limits
|
*/
...
|
*/
$config[ 'allowed_cors_origins' ] = [ APP_URL ];
