...
class Shpg extends FrontendController
{
    //
    public $CI;
    ...
    public function shpg_form($view_type = false) {
        ...
        if ( $this->input->post () ){
            ...
            $user_data['app_form_id']=APP_FORM_ID_SHPG;
            $app_form_id=APP_FORM_ID_SHPG;
            // Basic Details Start
            if($user_data['currentIndex']==0){
                // Basic Details API & Method
                $url = $api_urls[ 'shpg_basic_detail' ];
                $userdata = $this->session_userdata();
                $user_data['userdata'] = $userdata;
            ...
            if($user_data['currentIndex']==1){
                ...
                $url = $api_urls[ 'shpg_personal_detail' ];
                $userdata = $this->session_userdata();
                $user_data['userdata'] = $userdata;
                // Get Personal ApplicaNT ID
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            if($user_data['currentIndex']==2){
                $url = $api_urls[ 'shpg_parent_detail' ];
                $userdata = $this->session_userdata();
                $user_data['userdata'] = $userdata;
                // Get Personal ApplicaNT ID
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            $this->data['site_title'] = $this->data['page_title'] = 'Science and Humanities PG Application Form ';
            $this->template('applications/form_shpg', $this->data, true);
        }
    }
    public function shpg_preview() {
        $this->is_exists_user_logged();
        $applicant_id = $this->user_details_data['user_details']['data'][0]['applicant_personal_det_id'];
        $applicant_login_id=$this->user_details_data['user_details']['data'][0]['applicant_login_id'];
        // Get Form ID
        ...
        $this->data['user_data'] = $this->user_details_data['user_details']['data'][0];
        $this->data['personal_detail_list'] = $this->personal_detail_list($applicant_id);
        $this->data['site_title'] = $this->data['page_title'] = 'Science and Humanities PG Application Form Preview';
        /* applicant_address_details_list end */
        
        // Display page with the template function from REST_Controller
        $this->template('applications/shpg_preview', $this->data, true);
    }
    public function shpg_preference() {
        $api_urls = $this->config->item ( 'api_urls' );
        $url= $api_urls['shpg_preference'];
