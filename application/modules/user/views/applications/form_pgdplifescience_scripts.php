...
?>
<script>
var url = "<?php echo base_url(); ?>pgdplifescience";
...
<?php if($mode_edit) { ?>
   ...
   var url = "<?php echo base_url(); ?>pgdplifescience/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
...
var app_form_id = '<?php echo APP_FORM_ID_PGDPLIFE; ?>';
...
$(document).ready(function () {
	...
	<?php
	  ...
	  setTimeout(function() { 
		if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
			$("#save_exit").remove();
			if(mode_edit_url != ''){
				enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>advanced_pg_dp_form_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);				
			}else{
				enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>advanced_pg_dp_form_preview');
			}
		}
		if(curIndex==4){
    ...
    var settings = {
        ...
        onStepChanging: function (event, currentIndex, newIndex) { 
			...
			if(currentIndex < newIndex) {
				...
				if(currentIndex === 4) {
					...
					check_visible_input_validation('ob_pg_graduation','other_board_pg_graduation','<?php echo REQD_OTHER_UNIVERSITY_MSG; ?>',true,'<?php echo REQD_ALPHA_NAME_ONLY_MSG; ?>');
					...
					if(appering.isValid() && completed.isValid() && candidate_name.isValid() && academic_tenth_inst.isValid() && academic_board.isValid() &&  academic_marking_scheme.isValid() && academic_obtain_cgpa.isValid() && academic_yr_passing.isValid() && academic_reg_no.isValid() && academic_twelfth_inst.isValid() && academic_twelfth_board.isValid() && academic_marking_scheme_twelfth.isValid() && academic_obtain_cgpa_twelfth.isValid() && academic_yr_passing_twelfth.isValid() && academic_reg_no_twelfth.isValid() && academic_board_other.isValid() && academic_twelfth_board_other.isValid() && academic_graduation_inst.isValid() && academic_graduation_board.isValid() && academic_marking_scheme_graduation.isValid() && academic_obtain_cgpa_graduation.isValid() && academic_yr_passing_graduation.isValid() && academic_reg_no_graduation.isValid() && other_board_graduation.isValid() && academic_pg_graduation_inst.isValid() && academic_pg_graduation_board.isValid() && academic_marking_scheme_pg_graduation.isValid() && academic_obtain_cgpa_pg_graduation.isValid() && academic_yr_passing_pg_graduation.isValid() && academic_reg_no_pg_graduation.isValid() && other_board_pg_graduation.isValid()){
						...
						other_board_graduation.validate();
						other_board_pg_graduation.validate();
						return false; 						
					}
				}
        ...
        onStepChanged: function (event, currentIndex, priorIndex) { 
			...
			if(currentIndex == parseInt(total_wizard_Step - 1)){
				$("#save_exit").remove();
				if(mode_edit_url !=''){
					enable_preview_btn(currentIndex,'<?php echo base_url();?>advanced_pg_dp_form_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);					
				}else{
					enable_preview_btn(currentIndex,'<?php echo base_url();?>advanced_pg_dp_form_preview');	
				}
			}else{
				$("#previewbtn").remove();
			}
        ...
        onFinished: function (event, currentIndex) { 	
			...
			if(photograph.isValid() && signature.isValid() && applicant_name.isValid() && parent_name.isValid() && graduation_marksheet.isValid() && post_graduation_marksheet.isValid()) {
	...
	$("#pgdplifescience_form").steps(settings);
	//setsave exit attr	
    $(document).on("click", "#save_btn" , function() {
    	 $("#pgdplifescience_form").attr('isexit',1);
         $("#pgdplifescience_form").steps('next');
    });
    
    $('.actions a').click(function(){        	 
      $("#pgdplifescience_form").attr('isexit','');
    }); 
//end set attr
    ...
    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0)&& ($mode_edit == '')) { ?>
     $( ".steps li" ).each(function( index ) { 
      if(index<6){       	 
   	  $("#pgdplifescience_form-t-"+index).removeAttr("href");
     }			   
   	});
    <?php } ?>
	light_box_init();
	$(document).ready(function () {	
		...
		getSelect2('pd_campus','<?php echo base_url("get_apg_campus_preference"); ?>?prog_id='+pg_diploma_grad+url_edit,'Choose', formatRepoCommon,no_campus_msg, false, formatRepoSelection);
		...
		$('#pd_campus').on('change',function() {
			...
			getSelect2('pd_course_preference','<?php echo base_url("get_apg_course_preference"); ?>?pd_campus='+pd_campus+'&sort_by=id&sort_order=asc&prog_id='+pg_diploma_grad+url_edit,'Select Course Preferences', formatRepoCommon,no_course_msg, false, formatRepoSelection);			
		});
		...
		$('#completed').on('change',function(){
			if($(this).is(':checked')) {
				$('#td_pg_graduation_ms').show();
				$('#td_pg_graduation_cgpa').show();
				$('#td_pg_graduation_yop').show();
				$('#td_pg_graduation_reg_no').show();
				$('#academic_marking_scheme_pg_graduation').html('');
				$('#academic_obtain_cgpa_pg_graduation').val('');
				$('#academic_yr_passing_pg_graduation').val('');
				$('#academic_reg_no_pg_graduation').val('');
			} else {
				$('#td_pg_graduation_ms').hide();
				$('#td_pg_graduation_cgpa').hide();
				$('#td_pg_graduation_yop').hide();
				$('#td_pg_graduation_reg_no').hide();
			}
		});	
		<?php 
		...
		select2load('academic_pg_graduation_board','<?php echo $applicant_graduations_list[1]['univ_id']; ?>','<?php echo $applicant_graduations_list[1]['univ_name']; ?>');
		...
		select2load('academic_marking_scheme_pg_graduation','<?php echo $applicant_graduations_list[1]['mark_scheme_id']; ?>','<?php echo $applicant_graduations_list[1]['mark_scheme_name']; ?>');
...
function removeAttachedFile(dataThis) {
	...
	var applicant_appln_details_list_qual = '<?php echo $applicant_appln_details_list_qual; ?>';
	// Get Regenerated CSRF Dynamically
	var csrfHashRegenerate = $("input[name='"+csrfName+"']").val();

	if(applicant_appln_details_list_qual==2 && doc_id=="<?php echo DOCUMENT_ID_POST_GRADUATION_MARKSHEET; ?>"){
		required=true;
	}	
	
	if(applicant_appln_details_list_qual==1 && doc_id=="<?php echo DOCUMENT_ID_POST_GRADUATION_MARKSHEET; ?>"){
		required=false;
	}		
