...
<script>
    ...
    var app_form_id = '<?php echo APP_FORM_ID_PHD; ?>';
	...
	<?php if($mode_edit) { ?>
	   ...
	   var url = "<?php echo base_url(); ?>phd/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
	...
	var csrfName = '<?php echo $this->security->get_csrf_token_name(); ?>',
		 ...
		 <?php
		  ...
		  setTimeout(function() { 
			if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
				$("#save_exit").remove();
				if(mode_edit_url != ''){
					enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>phd-preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
				}else{				
					enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>phd-preview');
				}
			}
		...
		var settings = {
				...
				if(currentIndex < newIndex) {
					...
					if(currentIndex === 1) {
					  ...
					  if(pd_title.isValid() && pd_firstname.isValid() && pd_lastname.isValid() && pd_mobile_no.isValid() && pd_email.isValid() && pd_dob.isValid() && pd_gender.isValid() && pd_alt_email.isValid() && pd_blood_group.isValid() && pd_nationality.isValid() && pd_diffrently_abled.isValid() && pd_nature_deformity.isValid() && pd_percent_of_deformity.isValid() && pd_social_status.isValid() && pd_middlename.isValid()) {
						...
						var logged_applicant_id = $("#logged_applicant_id").val();
	        ...
	        onStepChanged: function (event, currentIndex, priorIndex) {
				...
				if(currentIndex == parseInt(total_wizard_Step - 1)){
					$("#save_exit").remove();
					if(mode_edit_url !=''){
						enable_preview_btn(currentIndex,'<?php echo base_url();?>phd-preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
					}else{
						enable_preview_btn(currentIndex,'<?php echo base_url();?>phd-preview');
					}					
				}else{
					$("#previewbtn").remove();
				}
    	...
    	$("#phd_form").steps(settings);
		//setsave exit attr	
	    $(document).on("click", "#save_btn" , function() {
	    	 $("#phd_form").attr('isexit',1);
	         $("#phd_form").steps('next');
        });
        
        $('.actions a').click(function(){        	 
          $("#phd_form").attr('isexit','');
        }); 
       //end set attr
	    ...
	    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0) && ($mode_edit == '')) { ?>
	     $( ".steps li" ).each(function( index ) { 
	      if(index<=4){       	 
	   	  $("#phd_form-t-"+index).removeAttr("href");
	     }			   
	   	});
	    <?php } ?>
		light_box_init();
