...
class Tamilperayam extends FrontendController
{
	//
    public $CI;
    ...
    public function tamilperayam_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		...
		// Get Form ID
		$get_form_id 		= APP_FORM_ID_TAMIL_PERAYAM;
		...
		if ( $this->input->post()){	
			...
			if($user_data['currentIndex']==0){
				...
				$url = $api_urls['basic_details_tamil'];
				...
				$user_data['deg_id'] 				= $user_data['tamilperayam_degree_list'];
				$user_data['course_pref'] 				= $user_data['tamil_course_preference'];
				$user_data['course_prefer_id'] 				= $user_data['course_prefer_id_1'];
				$user_data['choice_no'] 				= $user_data['course_choice_no_1'];
                ...
                $response 					     = array();
                $response['basic_details_tamil'] = $result;
			...
			if($user_data['currentIndex']==1){
				...
				$url = $api_urls['personal_detail_tamil'];
			...
			if($user_data['currentIndex']==2)
            {
                ...
                // Parent detail
                $api_urls 								= $this->config->item ( 'api_urls' );
                $url 									= $api_urls['tamil_parent_detail'];
                $method 								= 'POST';
                $userdata 								= $this->user_details_data;
                $user_data['userdata'] 					= $userdata;
                $user_data['applicant_personal_det_id'] = $applicant_id;
			...
			if($user_data['currentIndex']==4){
				...
				$api_urls 								= $this->config->item ('api_urls');
				$url 									= $api_urls['tamil_academic_detail'];
				$method 								= 'POST';
				$userdata 								= $this->user_details_data;
				$user_data['userdata'] 					= $userdata;
				$user_data['applicant_personal_det_id'] = $applicant_id;
			...
			if($user_data['currentIndex']==6){
				...
				$api_urls 							= $this->config->item ('api_urls');
                $url 							    = $api_urls['declaration_tamil'];
                $method 							= 'POST';
                $userdata 							= $this->user_details_data;
				$user_data['userdata'] 				= $userdata;
			...
			*/
			...
			$wizard_dt = $this->_get_wizard_details(APP_FORM_ID_TAMIL_PERAYAM);
            ...
            $applicant_course_prefer_details_list = $this->_get_applicant_tables(array('applicant_personal_det_id'=>$applicant_id,'applicant_appln_id'=>$applicant_appln_det_id), 'db_func_course_prefer_detail');
			...
			$this->data['site_title'] = $this->data['page_title'] = 'Tamil Perayam Programmes';
			$this->template('applications/form_tamilperayam', $this->data, true); 
		}        
    }
	
	// function to get the tamil perrayam degree list
	public function get_tamilperayam_degree_list() {
        parent::_get_tamilperayam_degree_list(false, $this->is_admin);
    }
	
	// Function tamil perayram degree based course selection
	public function get_tamilperayram_course_preference()
    {
		$degree_id = $this->input->get('tamil_degree_id');
        ...
        $api_urls = $this->config->item ('api_urls');
        $url = $api_urls['db_func_call_tamil_course'];
	...
	/*
	// Get Campus Preferences Of APGDLS
	public function get_apg_campus_preference()
    {
        ...
        $api_urls = $this->config->item ( 'api_urls' );
        $url = $api_urls['db_func_call_apg_campus'];
		$query = http_build_query($query_data);
        $url = $url .'?'.$query;
        $method = 'GET';
	...
	// Get Course Preferences Of APGDLS
	public function get_apg_course_preference()
    {
		$prog_id = $this->input->get('prog_id');
		$keywords=$this->input->get('keywords');
		$query_data = array();
        ...
        $api_urls = $this->config->item ( 'api_urls' );
        $url = $api_urls['db_func_call_apg_course'];
		$query = http_build_query($query_data);
        $url = $url .'?'.$query;
        $method = 'GET';
        $userdata = $this->session_userdata();
	...
	*/
	...
	// Tamilperayam Form Preview
    public function tamilperayam_form_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		$this->is_exists_user_logged();
		/*CRM ADMIN Edit form start*/
		$this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
		$this->data['mode_edit'] = $mode;
		...
		$this->data['user_data'] 				 = $this->user_details_data['user_details']['data'][0];
		$this->data['personal_detail_list'] 	 = $this->personal_detail_list($applicant_id);
		$this->data['site_title'] 				 = $this->data['page_title'] = 'Tamil Perayam Programmes Form Preview';

        // Display page with the template function from REST_Controller
        $this->template('applications/tamilperayam_form_preview', $this->data, true);
    }	
}