...
class Pgdplifescience extends FrontendController
{
	//
    public $CI;
    ...
    public function pgdplifescience_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		...
		// Get Form ID
		$get_form_id = APP_FORM_ID_PGDPLIFE;
		// Get Applicant ID
		...
		if ( $this->input->post () ){	
			...
			if($user_data['currentIndex']==0){
				$url = $api_urls['basic_details_apg'];
				// Method Type
				$method = 'POST';
                ...
                $response = array();
                $response['basic_details_apg'] = $result;
			...
			if($user_data['currentIndex']==1){
				$url = $api_urls['personal_detail_apg'];
				// Method Type
				$method = 'POST';
				$userdata = $this->user_details_data;
				$user_data['userdata'] = $userdata;
				...
				$user_data['applicant_personal_det_id'] = $applicant_id;
				//$user_data['splpref_id'] = $user_data['pd_course_preference'];
			...
			if($user_data['currentIndex']==2)
            {
                ...
                // Parent detail
                $api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'apg_parent_detail' ];
                $method = 'POST';
				$userdata = $this->user_details_data;
				$user_data['userdata'] = $userdata;
                $user_data['applicant_personal_det_id'] = $applicant_id;
			...
			if($user_data['currentIndex']==4){
				$api_urls = $this->config->item ( 'api_urls' );
				$url = $api_urls[ 'apg_academic_detail' ];
				$method = 'POST';
				$userdata = $this->user_details_data;
				$user_data['userdata'] = $userdata;
				$user_data['applicant_personal_det_id'] = $applicant_id;
				...
				$user_data['academic_obtain_cgpa_pg_graduation'] = ($user_data['academic_obtain_cgpa_pg_graduation']==null)?null:$user_data['academic_obtain_cgpa_pg_graduation'];
				$user_data['academic_yr_passing_pg_graduation'] = ($user_data['academic_yr_passing_pg_graduation']==null)?null:$user_data['academic_yr_passing_pg_graduation'];
				$user_data['academic_reg_no_pg_graduation'] = ($user_data['academic_reg_no_pg_graduation']==null)?null:$user_data['academic_reg_no_pg_graduation'];
			...
			$wizard_dt = $this->_get_wizard_details(APP_FORM_ID_PGDPLIFE);
			...
			$this->data['site_title'] = $this->data['page_title'] = 'Advanced PG DP Life Science ';
			$this->template('applications/form_pgdplifescience', $this->data, true); 
		}        
    }
	...
	// APGDLS Form Preview
	public function advanced_pg_dp_ls_form_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		$this->is_exists_user_logged();
		/*CRM ADMIN Edit form start*/
		$this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
		$this->data['mode_edit'] = $mode;
		...
		$this->data['user_data'] = $this->user_details_data['user_details']['data'][0];
		$this->data['personal_detail_list'] = $this->personal_detail_list($applicant_id);
		$this->data['site_title'] = $this->data['page_title'] = 'Advanced PG DP Life Science Form Preview';

        // Display page with the template function from REST_Controller
        $this->template('applications/pgdplifescience_form_preview', $this->data, true);
    }	
}