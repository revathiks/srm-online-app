...
var app_form_id = '<?php echo APP_FORM_ID_BSCAM; ?>';
...
<?php if($mode_edit) { ?>
   ...
   var url = "<?php echo base_url(); ?>bsc-am/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
...
<?php } else { ?>
   ...
   var url = "<?php echo base_url(); ?>bsc-am";
...
$(document).ready(function () {
	 ...
	 <?php
	  ...
	  setTimeout(function() { 
		if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
			$("#save_exit").remove();
			if(mode_edit_url != ''){
				enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>bscam_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
			}else{
				enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>bscam_preview');	
			}
		}
		if(curIndex==3){
    ...
    var settings = {
        ...
        onStepChanging: function (event, currentIndex, newIndex) { 
			...
			if(currentIndex < newIndex) {
				...
				if(currentIndex === 3) {
					...
					check_visible_input_validation('td_twelfth_yop','academic_yr_passing_twelfth','<?php echo REQD_YEAR_OF_PASSING_MSG; ?>',false);
        ...
        onStepChanged: function (event, currentIndex, priorIndex) { 
			...
			if(currentIndex == parseInt(total_wizard_Step - 1)){
				$("#save_exit").remove();
				if(mode_edit_url !=''){
					enable_preview_btn(currentIndex,'<?php echo base_url();?>bscam_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
				}else{
					enable_preview_btn(currentIndex,'<?php echo base_url();?>bscam_preview');
				}
			}else{
				$("#previewbtn").remove();
			} 		
	...
	$("#bsc_hons_am_form").steps(settings);
  //setsave exit attr	
    $(document).on("click", "#save_btn" , function() {
    	 $("#bsc_hons_am_form").attr('isexit',1);
         $("#bsc_hons_am_form").steps('next');
    });
    
    $('.actions a').click(function(){        	 
      $("#bsc_hons_am_form").attr('isexit','');
    }); 
//end set attr
  //to remove links from previous tabs a
    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0) && ($mode_edit == '')) { ?>
     $( ".steps li" ).each(function( index ) { 
      if(index<5){       	 
   	  $("#bsc_hons_am_form-t-"+index).removeAttr("href");
     }			   
   	});
    <?php } ?>
	light_box_init();
	$(document).ready(function () {
		...
		getSelect2('pd_hostel_accommodation','<?php echo base_url("get_hostel_accommodation"); ?>?sort_by=name&sort_order=asc','Choose Hostel Accommodation', formatRepoCommon,no_hostel_accommodation_msg, false, formatRepoSelection);
		...
		getSelect2('pd_father_phone_no_code','<?php echo base_url("get_mobile_codes"); ?>?user_login=1&sort_by=name&sort_order=asc','Choose Mobile Number Code', formatRepoCommon,no_mobile_code_msg, false, formatRepoMobileCodeSelection);
		...
		var no_course_msg = '<?php echo NO_SPL_PREFERENCE_COURSE_MSG; ?>';
		var no_campus_msg = '<?php echo NO_CAMPUS_MSG; ?>';
		
		getSelect2('pd_course_preference','<?php echo base_url("get_spl_preferences_bsc"); ?>?sort_by=name&sort_order=asc&prog_id='+null+url_edit,'Choose', formatRepoCommon,no_course_msg, false, formatRepoSelection);
		
		$("#main_div_course").hide();
		$('#pd_course_preference').on('change',function() {
			setEmptyOnChangeSelect2('pd_campus'); 
			// when no values return in json, add empty option value 
			if ($('#pd_campus').data('select2')) {$('#pd_campus').select2('destroy');}
			// make empty
			$('#pd_campus').html('');			
			$("#main_div_course").show();
			$("#custom-pd_campus-parsley-error").show();
			var pd_course_preference = $(this).val();
			console.log("pd_course_preference"+pd_course_preference);
			getSelect2('pd_campus','<?php echo base_url("get_campus_bsc"); ?>?prog_id='+null+'&sort_by=id&sort_order=asc'+url_edit,'Select Course Preference', formatRepoCommon,no_campus_msg, false, formatRepoSelection);			
		});		
		...
		$('#state').on('change',function() {
			...
			// when no values return in json, add empty option value 
			if ($('#district').data('select2')) {$('#district').select2('destroy');
				// make empty
				$('#district,#city').html(''); 	
			}			
		...
		<?php if($applicant_appln_details_list[0]['qualifyingexamfromindia']=='t') { ?>
			var value = 'yes';	
		<?php }else {  ?>
			var value = 'no';	
		<?php } ?>
		...
		<?php
			...
			if(empty($current_education_qual_status) || $current_education_qual_status=='f'){
				$cur_qual_completed = 'a';
			}else{
				$cur_qual_completed = 'c';
			}
		?>
		var cur_qual_completed = '<?php echo $cur_qual_completed; ?>';
		if(cur_qual_completed=="c"){
			$('#td_twelfth_ms').show();
			$('#td_twelfth_cgpa').show();
			$('#td_twelfth_yop').show();
			$('#td_twelfth_reg_no').show();
		}else if(cur_qual_completed=="a"){
			$('#td_twelfth_ms').hide();
			$('#td_twelfth_cgpa').hide();
			$('#td_twelfth_yop').hide();
			$('#td_twelfth_reg_no').hide();						
		...
		$('#country').on('change',function() {
    		...
    		if(country_value_default==country_phd_val){	
        		select2load('state_phd','<?php echo $state_id[0]; ?>','<?php echo $state_name[0]; ?>');
        		select2load('district_phd','<?php echo $district_id[0]; ?>','<?php echo $district_name[0]; ?>');
        		select2load('city_phd','<?php echo $city_id[0]; ?>','<?php echo $city_name[0]; ?>');
    		}
		});			
		...
		onchangeLableHide('studied_in_india');
		onchangeLableHide('pd_course_preference');
		...
		onchangeLableHide('country');
		onchangeLableHide('state');
		onchangeLableHide('district');
		onchangeLableHide('city');
