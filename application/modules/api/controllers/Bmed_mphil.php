 ...
 *
 */
class Bmed_mphil extends API_Controller 
{
	...
	//B-M-ed Mphil api start
    public function bmed_basic_detail_post() {
        ...
        if($applicant_appln_det_res) {
            if($applicant_appln_det_res['status'] == 'error') {
                ...
                $wizard_update = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id , FORM_WIZARD_BASIC_ID);
    ...
    public function bmed_personal_detail_post() {
        $applicant_personal_det_table = APPLICANT_PERSONAL_DET_TABLE;
        ...
        $course_prefer_id = $this->post('course_prefer_id_1',true);
        ...
        if($course_pref) {
            ...
            $_POST['active'] = TRUE;
            if($course_prefer_id) {
                $_POST['applicant_course_prefer_id'] = $course_prefer_id;
            }
            $_POST['course_id'] = $course_pref;
            $_POST['choice_no'] = $course_choice_no;
            $_POST['applicant_appln_id'] = $applicant_appln_id;
            if($course_prefer_id) {
                $course_res=$this->_insert_update_applicant_tables($function_name, array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id), $table_schema, $applicant_course_prefer_table, $applicant_course_prefer_columns, 1, 'applicant_course_prefer_id', $course_prefer_id);
            }else {
                $chk_flag = false;
                if(!$course_pref) {
                    $chk_flag = true;
    ...
    //parent
    public function bmed_parent_detail_post()
    {
        $app_form_id = APP_FORM_ID_BMED_MPHIL;
        ...
        $table_parent_details = $table_schema.".".$applicant_parent_det_table;
       //if($add_guardian_details!="on" &&  $add_guardian_details !=="false" && ($add_guardian_details==1 || $add_guardian_details=="true" || $add_guardian_details==true)){
    ...
    //address
    public function bmed_addressdet_post()
    {
    ...
    //declaration
    public function bmed_declaration_post() {
        $app_form_id = APP_FORM_ID_BMED_MPHIL;
        $applicant_personal_det_table = APPLICANT_PERSONAL_DET_TABLE;
        $applicant_appln_det_table = APPLICANT_APPLN;
        $table_schema = SCHEMA_ADMISSION;
        ...
        if($applicant_personal_det_res){
            $wizard_update = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id , FORM_WIZARD_DECLARATION_ID);
        }
        $this->response ($applicant_personal_det_res , API_Controller::HTTP_OK);
    }
    //end declaration
    public function bmed_mphil_courses_get() {
        $table_schema = SCHEMA_ADMISSION;
    ...
    public function bmed_academicdet_post()
    {
        $user_data = $this->post();
        $table_schema = SCHEMA_ADMISSION;
        $applicant_personal_det_table = APPLICANT_PERSONAL_DET_TABLE;
       ...
       // $_POST['updated_by'] = $applicant_personal_det_id;
        ...
        if($current_qualification_status=="ug_pass" || $current_qualification_status=="pg_pur" || $current_qualification_status=="pg_pass")
        {
        ...
        {
            $this->form_validation->set_rules('institute_name_pg', 'PG Institute Name','trim|required');
            $this->form_validation->set_rules('university_pg', 'PG University','trim|required');
            $this->form_validation->set_rules('yr_passing_pg', ' PG Year of Passing','trim|required');
            $this->form_validation->set_rules('register_no_pg', 'PG Roll number','trim|required');
            $this->form_validation->set_rules('degree_pg', 'PG Degree','trim|required');
            if($current_qualification_status=="pg_pass")
            {
                $this->form_validation->set_rules('marking_scheme_pg', ' PG Marking Scheme','trim|required');
                $this->form_validation->set_rules('obtained_percentage_cgpa_pg', 'PG Obtained Percentage/CGPA','trim|required');
            }
        }
        ...
        //for 12th
            ...
            $stream_name = ($user_data['stream_name']!='' && $user_data['stream_name']!='null') ? $user_data['stream_name'] :NULL;
           ...
           // }
            ...
            $_POST['is_curr_qualify'] = $cur_qual_completed_twelfth;
        ...
        $marking_scheme_ug = ($user_data['marking_scheme_ug']!='' && $user_data['marking_scheme_ug']!='null') ? $user_data['marking_scheme_ug']:NULL;
        $obtained_percentage_cgpa_ug = ($user_data['obtained_percentage_cgpa_ug']!='' && $user_data['obtained_percentage_cgpa_ug']!='null') ?$user_data['obtained_percentage_cgpa_ug'] :NULL;
        ...
        if(isset($current_qualification_status) && ($current_qualification_status==="ug_pur")){
        ...
        //end ug        
        if(isset($current_qualification_status) && ($current_qualification_status=="ug_pur" || $current_qualification_status=="ug_pass")){
            ...
            $tbl_schema_both = $table_schema.'.'.APPLICANT_GRADUATION_TABLE;
                ...
                $where_pg_gno['graduation_type_id'] = GRADUATION_PG_ID;
        ...
        $marking_scheme_pg = ($user_data['marking_scheme_pg']!='' && $user_data['marking_scheme_pg']!='null') ? $user_data['marking_scheme_pg']:NULL;
        $obtained_percentage_cgpa_pg = ($user_data['obtained_percentage_cgpa_pg']!='' && $user_data['obtained_percentage_cgpa_pg']!='null') ?$user_data['obtained_percentage_cgpa_pg'] :NULL;
       ...
       // }
        ...
        //end new
        $this->_get_error_status($schooling_res);
        $this->_get_error_status($graduation_res);
