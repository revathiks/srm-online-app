 ...
 *
 */
class Agrisci extends API_Controller {
	...
	public function basic_details_agrisci_post(){
		$app_form_id_agrisci = APP_FORM_ID_AGRISCI;
		...
		// Columns
		$applicant_appln_columns = 'qualifyingexamfromindia,active,is_agree,application_status_id';
		// Post Data
		...
		$applicant_appln_det_res = $this->_get_appln_form_id($applicant_personal_det_id,$app_form_id_agrisci);
		...
		if($qualifying_exam_from_india) {
			if($qualifying_exam_from_india['status'] == 'error') {
				...
				// Update Wizard ID
				$response['forn_w_wizard_id'] = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_agrisci , FORM_WIZARD_BASIC_ID);				
	...
	public function personal_detail_agrisci_post(){
		$app_form_id_agrisci = APP_FORM_ID_AGRISCI;
		$table_schema = SCHEMA_ADMISSION;
		$applicant_course_prefer_table = APPLICANT_COURSE_PREFER_TABLE;
		$applicant_campus_prefer_table = APPLICANT_APPLN;
		$applicant_personal_det_table = APPLICANT_PERSONAL_DET_TABLE;
		...
		$applicant_campus_det_columns = 'grad_id,active';
		...
		$pd_program_level_id = $this->post('pd_program_level_id',true);
		...
		$this->form_validation->set_rules('pd_program_level_id', 'Campus','trim|required');
		...
		$_POST['grad_id'] = $pd_program_level_id;
		...
		$spec_pref_res['applicant_program_level'] = $this->_insert_update_applicant_tables($function_name, array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id), $table_schema, $applicant_campus_prefer_table, $applicant_campus_det_columns, 2);
		...
		// Update Wizard ID
		$spec_pref_res['forn_w_wizard_id'] = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_agrisci , FORM_WIZARD_PERSONAL_ID);
		
		$this->response ($spec_pref_res , API_Controller::HTTP_OK );
	}		
	
	// Get Agrisci Program Level
	public function db_func_call_agrisci_pgm_level_get() {
		$form_id = APP_FORM_ID_AGRISCI;
	...
	// Get Agrisci Course Preferences
	public function db_func_call_agrisci_course_get() {
	...
	public function agrisci_parent_detail_post() {
    	$app_form_id = APP_FORM_ID_AGRISCI;
		...
		// Wizard ID Update
		$parent_res['wizard_id_update'] = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_AGRISCI , FORM_WIZARD_PARENT_ID);
	...
	public function agrisci_academic_detail_post() {
    	$app_form_id_agrisci = APP_FORM_ID_AGRISCI;
		...
		$this->form_validation->set_data($this->post());
		$this->form_validation->set_rules('applicant_personal_det_id', 'Personal det id','trim|required');
		$this->form_validation->set_rules('curr_edu_quali_status', 'Current education qualification status','trim|required');
		$this->form_validation->set_rules('name_as_in_marksheet', 'Name as in marksheet','trim|required');
		$this->form_validation->set_rules('tenth_institute_name', 'Tenth institute name','trim|required');
		$this->form_validation->set_rules('tenth_board_id', 'Tenth board','trim|required');
		$this->form_validation->set_rules('tenth_msc_id', 'Tenth Marking Scheme','trim|required');
		$this->form_validation->set_rules('tenth_cgpa', 'Tenth CGPA','trim|required');
		$this->form_validation->set_rules('tenth_yop', 'Tenth YOP','trim|required');
		$this->form_validation->set_rules('tenth_rollno', 'Tenth YOP','trim|required');
		
		// Twelfth SSV
		$this->form_validation->set_rules('twelfth_institute_name', 'Twelfth institute name','trim|required');
		$this->form_validation->set_rules('twelfth_board_id', 'Twelfth board','trim|required');
		$this->form_validation->set_rules('twelfth_yop', 'Twelfth YOP','trim|required');		
		...
		if($curr_edu_quali_status=='1'){
			...
			$this->form_validation->set_rules('twelfth_institute_name', 'Twelfth institute name','trim|required');
			$this->form_validation->set_rules('twelfth_board_id', 'Twelfth board','trim|required');
			$this->form_validation->set_rules('twelfth_rollno', 'Twelfth YOP','trim|required');				
			$this->form_validation->set_rules('twelfth_msc_id', 'Twelfth Marking Scheme','trim|required');
			$this->form_validation->set_rules('twelfth_cgpa', 'Twelfth CGPA','trim|required');
			$this->form_validation->set_rules('twelfth_yop', 'Twelfth YOP','trim|required');			
			$curr_edu_quali_status = 2;
			$is_curr_qualify=true;
		...
		// Wizard ID Update
		$parent_res['wizard_id_update'] = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_AGRISCI , FORM_WIZARD_ACADEMIC_ID);
	...
	public function declaration_agrisci_post(){
		...
		if($user_data['currentIndex']==6){
			...
			$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_AGRISCI , FORM_WIZARD_UPLOAD_DECLARATION_ID);
