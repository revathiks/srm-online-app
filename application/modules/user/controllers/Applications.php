...
class Applications extends FrontendController
{
    ...
    public function phd_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
        ...
        if ( $this->input->post () ){	
			...
			// $this->data['return_admin'] = $user_type;
            $api_urls = $this->config->item ( 'api_urls' );
            ...
            $wizard_dt = $this->_get_wizard_details(APP_FORM_ID_PHD);
    ...
    public function upload_file() {
		...
		if(!$mode){
			$logged_applicant_id = $this->input->post('applicant_id',true);
		}
        ...
        $ses_login_id=$this->user_details_data['user_details']['data'][0]['applicant_login_id'];
        ...
        $app_login_id = $this->input->post('applicant_login_id');
        $applicant_login_id = ($app_login_id)?$app_login_id:$ses_login_id;
        ...
        } else { $userdata = $this->session_userdata();
         }
    ...
    public function del_uploaded_file() {
        ...
        if($result_token['valid']=='true')
        {
            ...
            $result = $this->call_api ( $url , $method , $data );
			// Inject new csrf token and assing in $result['token']
    ...
    public function phd_form_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
        $this->is_exists_user_logged();
		/*CRM ADMIN Edit form start*/
		$this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
		$this->data['mode_edit'] = $mode;
        ...
        // Display page with the template function from REST_Controller
        $this->template('applications/form_preview', $this->data, true);
    }
	...
	public function thank_you() {
        ...
        $applicant_login_id = ($this->input->get('applicant_login_id'))?($this->input->get('applicant_login_id')):$ses_login_id;
        ...
        $this->data['applicant_login_id'] = $applicant_login_id;
