...
var app_form_id = '<?php echo APP_FORM_ID_DE; ?>';
...
<?php if($mode_edit) { ?>
   ...
   var url = "<?php echo base_url(); ?>dis-education/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
...
<?php } else { ?>
   ...
   var url = "<?php echo base_url(); ?>dis-education";
...
var get_percentage_url = '<?php echo base_url(); ?>/user/get_percentage_w_tab?'+url_edit;
...
$(document).ready(function () {
	 ...
	 <?php
	  ...
	  setTimeout(function() { 
		if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
			$("#save_exit").remove();
			if(mode_edit_url != ''){
				enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>dde_form_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);				
			}else{
				enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>dde_form_preview');
			}
		}
		if(curIndex==4){
    ...
    var settings = {
			...
			if(currentIndex < newIndex) {
				...
				if(currentIndex === 5) {
					...
					if(bank_name.isValid() && branch_name.isValid() && ddnumber.isValid() && ddate.isValid() && online.isValid() && dd.isValid()){
						...
						var user_data = 'bank_name='+bank_name+'&branch_name='+branch_name+'&dd_cheque_no='+dd_cheque_no+'&dd_cheque_date='+dd_cheque_date+'&payment_mode='+payment_mode+'&'+csrfName+'='+csrfHash;		
						var moveNxt=0;
					    <?php if(empty($payment_details_list)) { ?> 
						$.ajax({
							type: 'POST',
        ...
        onStepChanged: function (event, currentIndex, priorIndex) { 
			...
			if(currentIndex == parseInt(total_wizard_Step - 1)){
				$("#save_exit").remove();
				if(mode_edit_url !=''){
					enable_preview_btn(currentIndex,'<?php echo base_url();?>dde_form_preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);					
				}else{
					enable_preview_btn(currentIndex,'<?php echo base_url();?>dde_form_preview');
				}
			}else{
				$("#previewbtn").remove();
			}
	...
	$("#dist_edu_form").steps(settings);
	//setsave exit attr	
    $(document).on("click", "#save_btn" , function() {
    	 $("#dist_edu_form").attr('isexit',1);
         $("#dist_edu_form").steps('next');
    });
    
    $('.actions a').click(function(){        	 
      $("#dist_edu_form").attr('isexit','');
    }); 
//end set attr
    ...
    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0) && ($mode_edit == '')) { ?>
     $( ".steps li" ).each(function( index ) { 
      if(index<6){       	 
   	  $("#dist_edu_form-t-"+index).removeAttr("href");
     }			   
   	});
    <?php } ?>
	light_box_init();
	 $(document).ready(function () {
		...
		$('#pd_program').on('change',function() {
			console.log("pd_program "+$(this).val());
