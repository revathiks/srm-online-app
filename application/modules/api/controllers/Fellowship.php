 ...
 *
 */
class Fellowship extends API_Controller {
	...
	public function basic_details_fellowship_post(){
		$app_form_id_fellowship = APP_FORM_ID_FELLOWSHIP;
		...
		$applicant_appln_det_res = $this->_get_appln_form_id($applicant_personal_det_id,$app_form_id_fellowship);
		...
		if($qualifying_exam_from_india) {
			if($qualifying_exam_from_india['status'] == 'error') {
				...
				// Update Wizard ID
				$response['forn_w_wizard_id'] = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_fellowship , FORM_WIZARD_BASIC_ID);				
	...
	public function personal_detail_fellowship_post(){
		$app_form_id_fellowship = APP_FORM_ID_FELLOWSHIP;
		$table_schema = SCHEMA_ADMISSION;
		...
		// Preference Detail
		$applicant_personal_det_id = $this->post('applicant_personal_det_id',true);
		//$splpref_id = $this->post('splpref_id',true);
		...
		// Update Wizard ID
		$spec_pref_res['forn_w_wizard_id'] = $this->common_wizardupdate($applicant_personal_det_id , $app_form_id_fellowship , FORM_WIZARD_PERSONAL_ID);
		
		$this->response ($spec_pref_res , API_Controller::HTTP_OK );
	}		
	
	// Fellowship Select Course and Campus Preference Dropdown
	public function db_func_call_fellowship_course_get() {
		$get_prog_id = $this->input->get('prog_id');
		$is_course =  $this->get('is_course',true);
		$form_id = APP_FORM_ID_FELLOWSHIP;
	...
	public function db_func_call_fellowship_campus_get() {
		$get_prog_id = $this->input->get('prog_id');
		$form_id = APP_FORM_ID_FELLOWSHIP;
		$fn_get_app_course_detail = FN_GET_CHOICE_DROPDOWN;
		$table_schema = SCHEMA_ADMISSION;
	...
	public function fellowship_parent_detail_post() {
    	$app_form_id = APP_FORM_ID_FELLOWSHIP;
		...
		// Wizard ID Update
		$parent_res['wizard_id_update'] = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_FELLOWSHIP , FORM_WIZARD_PARENT_ID);
	...
	public function fellowship_academic_detail_post() {
    	$app_form_id_fellowship = APP_FORM_ID_FELLOWSHIP;
		...
		$addq_ids = $this->post('publn_det_ids',true);
		$addq_ids = json_decode($addq_ids,true);
		...
		if($curr_edu_quali_status=='aug'){
			$curr_edu_quali_status = 1;
			$result_declared_ug = false;
			$is_curr_qualify_ug = false;			
		}elseif($curr_edu_quali_status=='apg'){
			$curr_edu_quali_status = 2;	
			$result_declared_pg = false;
			$is_curr_qualify_pg = false;			
		}elseif($curr_edu_quali_status=='cug'){
			$curr_edu_quali_status = 3;	
			$result_declared_ug = true;
			$is_curr_qualify_ug = true;
		}elseif($curr_edu_quali_status=='cpg'){
			$curr_edu_quali_status = 4;	
			$result_declared_pg = true;
			$is_curr_qualify_pg = true;			
		}

		$_SERVER["REQUEST_METHOD"] = "POST";
		$_POST['qual_status_id'] = $curr_edu_quali_status;
		// Columns
		$applicant_appln_columns = 'qual_status_id';
		$applicant_appln_table = APPLICANT_APPLN;
		$qualifying_exam_from_india = $this->_insert_update_applicant_tables($function_name, array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id), $table_schema, $applicant_appln_table, $applicant_appln_columns, 2);
		...
		// Graduation Detail
		$academic_graduation_inst = $user_data['academic_graduation_inst'];
		$academic_graduation_board = $user_data['academic_graduation_board'];		
		if($curr_edu_quali_status==1){
			$academic_marking_scheme_graduation = null;	
			$academic_obtain_cgpa_graduation = null;
			$academic_yr_passing_graduation = null;
			$academic_reg_no_graduation = null;
			$other_board_graduation = null;
		}else{
			$academic_marking_scheme_graduation = $user_data['academic_marking_scheme_graduation'];	
			$academic_obtain_cgpa_graduation = $user_data['academic_obtain_cgpa_graduation'];
			$academic_yr_passing_graduation = $user_data['academic_yr_passing_graduation'];
			$academic_reg_no_graduation = $user_data['academic_reg_no_graduation'];
			$other_board_graduation = $user_data['other_board_graduation'];
		}
		$is_graduation = $user_data['is_graduation'];
		
		// PG Graduation Detail
		$academic_pg_graduation_inst = $user_data['academic_pg_graduation_inst'];
		$academic_pg_graduation_board = $user_data['academic_pg_graduation_board'];
		if($curr_edu_quali_status==2){
			$academic_marking_scheme_pg_graduation = null;	
			$academic_obtain_cgpa_pg_graduation = null;
			$academic_yr_passing_pg_graduation = null;
			$academic_reg_no_pg_graduation = null;
			$other_board_pg_graduation = null;
		}else{
			$academic_marking_scheme_pg_graduation = $user_data['academic_marking_scheme_pg_graduation'];	
			$academic_obtain_cgpa_pg_graduation = $user_data['academic_obtain_cgpa_pg_graduation'];
			$academic_yr_passing_pg_graduation = $user_data['academic_yr_passing_pg_graduation'];
			$academic_reg_no_pg_graduation = $user_data['academic_reg_no_pg_graduation'];
			$other_board_pg_graduation = $user_data['other_board_pg_graduation'];
		}		
		$is_pg_graduation = $user_data['is_pg_graduation'];		
		
		$graduation_table = APPLICANT_GRADUATION_TABLE;
		
		$grad_columns = "insti_name,univ_id,mark_scheme_id,mark_percent,completion_year,registration_no,graduation_type_id,created_by,updated_by,is_curr_qualify,applicant_appln_id,result_declared,applicant_personal_det_id,active,other_university_name";
		
		if($is_graduation){
			$this->academic_ug_pg_common($graduation_table,$table_schema,$grad_columns,$applicant_personal_det_id,$applicant_appln_id,UG_GRAD,'applicant_grad_det_id',$academic_graduation_inst,$academic_graduation_board,$academic_marking_scheme_graduation,$academic_obtain_cgpa_graduation,$academic_yr_passing_graduation,$academic_reg_no_graduation,$is_curr_qualify_ug,$result_declared_ug,$other_board_graduation,$created_by,$updated_by);
			$parent_res['graduation'] = array("status"=>true,"type"=>"graduation");
		}		
		
		if($curr_edu_quali_status==4 || $curr_edu_quali_status==2){
			$this->academic_ug_pg_common($graduation_table,$table_schema,$grad_columns,$applicant_personal_det_id,$applicant_appln_id,PG_GRAD,'applicant_grad_det_id',$academic_pg_graduation_inst,$academic_pg_graduation_board,$academic_marking_scheme_pg_graduation,$academic_obtain_cgpa_pg_graduation,$academic_yr_passing_pg_graduation,$academic_reg_no_pg_graduation,$is_curr_qualify_pg,$result_declared_pg,$other_board_pg_graduation,$created_by,$updated_by);
			$parent_res['pg_graduation'] = array("status"=>"Record added successfully","type"=>"pg graduation");
		}else{
			$whereparentgno = array();
			$whereparentgno['applicant_personal_det_id'] = $applicant_personal_det_id;
			$whereparentgno['graduation_type_id'] = PG_GRAD;
			$whereparentgno['applicant_appln_id'] = $applicant_appln_id;
			$whereparentgno['table_schema'] = SCHEMA_ADMISSION;
			$this->common->common_delete_new ( $graduation_table , $whereparentgno );		
		}		
		...
		if($tenth_scholling_det_id){
			$is_curr_qualify_tenth = false;
			$result_declared_tenth = true;
			$this->academic_tenth_twelfth_common($applicant_schooling_det_table,$table_schema,$columns,$applicant_personal_det_id,$applicant_appln_id,$name_as_in_marksheet,$tenth_institute_name,$tenth_board_id,$tenth_msc_id,$tenth_cgpa,$tenth_yop,$tenth_rollno,$tenth_scholling_det_id,$academic_board_other,$is_curr_qualify_tenth,$result_declared_tenth,$created_by,$updated_by);
			$parent_res['tenth'] = array("status"=>true,"type"=>"tenth schooling");
		}

		if($twelfth_scholling_det_id){
			$is_curr_qualify = false;
			$result_declared = true;
			$this->academic_tenth_twelfth_common($applicant_schooling_det_table,$table_schema,$columns,$applicant_personal_det_id,$applicant_appln_id,$name_as_in_marksheet,$twelfth_institute_name,$twelfth_board_id,$twelfth_msc_id,$twelfth_cgpa,$twelfth_yop,$twelfth_rollno,$twelfth_scholling_det_id,$academic_twelfth_board_other,$is_curr_qualify,$result_declared,$created_by,$updated_by);
			$parent_res['twelfth'] = array("status"=>true,"type"=>"twelfth schooling");
		}
		
		$additional_columns = "applicant_grad_det_id,program_name,is_addi_qualification,completion_year,active,applicant_appln_id,applicant_personal_det_id";

		if($journal_conf_name) {
			foreach($journal_conf_name as $k=>$v) {
				$n_addq_ids = $addq_ids[$k];
				...
				$_POST = array();
				if($n_addq_ids) {
					$_POST['applicant_grad_det_id'] = $n_addq_ids;        
				}
				$_POST['program_name'] = ($n_journal_conf_name=='')?null:$n_journal_conf_name;
				$_POST['is_addi_qualification'] = true;
				$_POST['completion_year'] = ($n_year=='')?null:$n_year;;
				$_POST['active'] = TRUE;
				$_POST['applicant_appln_id'] = $applicant_appln_id;
				$_POST['applicant_personal_det_id'] = $applicant_personal_det_id;
				if($n_addq_ids) {
					$parent_res[] = $this->_insert_update_applicant_tables($function_name, array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id,'is_addi_qualification'=>true), $table_schema, $graduation_table, $additional_columns, 1, 'applicant_grad_det_id', $n_addq_ids);
				} else {
					$chk_flag = false;
					if(!$n_journal_conf_name && !$n_year) {
							$chk_flag = true;
					}
					if(!$chk_flag) {
							$parent_res[] = $this->_insert_update_applicant_tables($function_name, array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id,'is_addi_qualification'=>true), $table_schema, $graduation_table, $additional_columns, 1, 'applicant_grad_det_id');
					}
				}
			}
		}		
		...
		// Wizard ID Update
		$parent_res['wizard_id_update'] = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_FELLOWSHIP , FORM_WIZARD_ACADEMIC_ID);
	...
	public function declaration_fellowship_post(){
		...
		if($user_data['currentIndex']==6){
			...
			$wizard_update = $this->common_wizardupdate($applicant_id , APP_FORM_ID_FELLOWSHIP , FORM_WIZARD_UPLOAD_DECLARATION_ID);
	...
	public function academic_tenth_twelfth_common($applicant_schooling_det_table,$table_schema,$columns,$applicant_personal_det_id,$applicant_appln_id,$name_as_in_marksheet,$institute_name,$board_id,$msc_id,$cgpa,$yop,$rollno,$schooling_id,$board_other,$is_curr_qualify,$result_declared,$created_by,$updated_by){
		
		$check_applicant_id_sd = $this->check_exist_user($applicant_schooling_det_table, $table_schema,$columns,  array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id,'schooling_id'=>$schooling_id),1,'applicant_scl_det_id');
		...
		if($check_applicant_id_sd['data'][0]['applicant_appln_id']==$applicant_appln_id){
			...
			$put_arrayschool['result_declared'] = $result_declared;
			$put_arrayschool['name_as_in_marksheet'] = $name_as_in_marksheet;
			$put_arrayschool['institute_name']= $institute_name;
			$put_arrayschool['board_id'] = $board_id;
			$put_arrayschool['marking_scheme_id'] = $msc_id;
			$put_arrayschool['mark_percentage'] = $cgpa;
			$put_arrayschool['completion_year'] = $yop;
			$put_arrayschool['registration_no'] = $rollno;
			$put_arrayschool['schooling_id'] = $schooling_id;
			if($board_other){
				$put_arrayschool['other_board_name'] = $board_other;
			}
			$put_arrayschool['is_curr_qualify'] = $is_curr_qualify;
			...
			$_POST['applicant_personal_det_id']= $applicant_personal_det_id;
			$_POST['applicant_appln_id']= $applicant_appln_id;
			$_POST['result_declared'] = $result_declared;
			$_POST['name_as_in_marksheet'] = $name_as_in_marksheet;
			$_POST['institute_name']= $institute_name;
			$_POST['board_id'] = $board_id;
			$_POST['marking_scheme_id'] = $msc_id;
			$_POST['mark_percentage'] = $cgpa;
			$_POST['completion_year'] = $yop;
			$_POST['registration_no'] = $rollno;
			$_POST['schooling_id'] = $schooling_id;
			$_POST['active']= true;
			$_POST['created_by']= $created_by;
			if($board_other){
				$_POST['other_board_name'] = $board_other;
			}
	...
	public function academic_ug_pg_common($graduation_table,$table_schema,$grad_columns,$applicant_personal_det_id,$applicant_appln_id,$graduation_type_id,$applicant_grad_det_id,$academic_graduation_inst,$academic_graduation_board,$academic_marking_scheme_graduation,$academic_obtain_cgpa_graduation,$academic_yr_passing_graduation,$academic_reg_no_graduation,$is_curr_qualify_ug,$result_declared_ug,$other_board_graduation,$created_by,$updated_by){
		
		$graduation_check = $this->check_exist_user($graduation_table, $table_schema,$grad_columns,  array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id,'graduation_type_id'=>$graduation_type_id),1,$applicant_grad_det_id);
		if($graduation_check['data'][0]['applicant_appln_id']==$applicant_appln_id){
			$wheregraduation['applicant_personal_det_id'] = $applicant_personal_det_id;
			$wheregraduation['applicant_appln_id'] = $applicant_appln_id;
			$wheregraduation['graduation_type_id'] = $graduation_type_id;
			$put_arraygraduation['insti_name'] = $academic_graduation_inst;
			$put_arraygraduation['univ_id'] = $academic_graduation_board;
			$put_arraygraduation['mark_scheme_id'] = $academic_marking_scheme_graduation;
			$put_arraygraduation['mark_percent'] = $academic_obtain_cgpa_graduation;
			$put_arraygraduation['completion_year'] = $academic_yr_passing_graduation;
			$put_arraygraduation['registration_no'] = $academic_reg_no_graduation;
			$put_arraygraduation['graduation_type_id'] = $graduation_type_id;
			$put_arraygraduation['applicant_appln_id']=$applicant_appln_id;
			$put_arraygraduation['updated_by']=$updated_by;
			$put_arraygraduation['is_curr_qualify'] = $is_curr_qualify_ug;
			$put_arraygraduation['result_declared']=$result_declared_ug;
			$put_arraygraduation['applicant_personal_det_id']=$applicant_personal_det_id;
			if($other_board_graduation){
				$put_arraygraduation['other_university_name'] = $other_board_graduation;
			}				
			$this->common->common_update ($table_schema.'.'.$graduation_table ,'' , $put_arraygraduation, $wheregraduation);
		}else{
			$_POST['insti_name'] = $academic_graduation_inst;
			$_POST['univ_id'] = $academic_graduation_board;
			$_POST['mark_scheme_id'] = $academic_marking_scheme_graduation;
			$_POST['mark_percent'] = $academic_obtain_cgpa_graduation;
			$_POST['completion_year'] = $academic_yr_passing_graduation;
			$_POST['registration_no'] = $academic_reg_no_graduation;
			$_POST['graduation_type_id'] = $graduation_type_id;
			$_POST['applicant_appln_id']=$applicant_appln_id;
			$_POST['created_by']=$created_by;
			$_POST['is_curr_qualify'] = $is_curr_qualify_ug;
			$_POST['result_declared']=$result_declared_ug;
			$_POST['active']=true;
			$_POST['applicant_personal_det_id']=$applicant_personal_det_id;
			if($other_board_graduation){
				$_POST['other_university_name'] = $other_board_graduation;
			}
			
			$function_name = $this->get_function_name ( __FUNCTION__ );      
			$this->_common_insert ( $graduation_table , $table_prefix , $function_name , $grad_columns , $table_schema);
		}
	}	
} 