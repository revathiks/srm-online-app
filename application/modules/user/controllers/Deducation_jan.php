...
class Deducation_jan extends FrontendController
{
	//
    public $CI;
    ...
    public function dist_edu_jan_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
        $this->is_exists_user_logged();
		/*CRM ADMIN Edit form start*/
		...
		// Get Form ID
		$get_form_id = APP_FORM_ID_DE_JAN;
		// Get Applicant ID
		...
		if ( $this->input->post () ){	
			...
			if($user_data['currentIndex']==0){
				...
				$url = $api_urls[ 'wizard_api_dist_edu_jan' ];
			...
			if($user_data['currentIndex']==2){
				...
				$url = $api_urls[ 'wizard_api_parent_edu' ];
				$userdata = $this->user_details_data;
				$user_data['userdata'] = $userdata;		
				// Get Personal Applicant ID
				$user_data['applicant_id'] = $applicant_id;
			...
			if($user_data['currentIndex']==4){
				$api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'dis_edu_academic_jan' ];
                $method = 'POST';
				$userdata = $this->user_details_data;
				$user_data['userdata'] = $userdata;
				$user_data['applicant_id'] = $applicant_id;
			...
			if($user_data['currentIndex']==6){
				$api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'declaration_dde_jan' ];
                $method = 'POST';
				$userdata = $this->user_details_data;
				$user_data['userdata'] = $userdata;
				$user_data['applicant_id'] = $applicant_id;
			...
			$this->template('applications/dis_edu_jan_form', $this->data, true);
		}
    }		
	...
	public function get_percentage_w_tab()
    {
		...
		$get_form_id = ($get_form_id)?$get_form_id:APP_FORM_ID_DE_JAN;
		// Get Applicant ID
    ...
    public function dde_jan_form_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
		$this->is_exists_user_logged();
		/*CRM ADMIN Edit form start*/
		$this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
		$this->data['mode_edit'] = $mode;
        ...
        // Display page with the template function from REST_Controller
        $this->template('applications/dis_edu_jan_form_preview', $this->data, true);
    }	
