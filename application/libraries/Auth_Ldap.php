...
class Auth_Ldap {
    ...
    private function _authenticate($username, $password) {
        ...
        if(!$bind){
           // log_message('error', 'Unable to perform anonymous/proxy bind');
            //show_error('Unable to bind for user id lookup');
             $customdata = array('username' => "Failed login attempt: ".$this->proxy_user,
                            'logged_in' => FALSE);
             return $customdata;
        }
        ...
        if(! $bind) {
            $customdata = array('username' => "Failed login attempt: ".$username,
                            'logged_in' => FALSE);
        return $customdata;
        ...
        return array('cn' => $cn, 'username' => $username,'logged_in' => true);
        }
        else
        {
            $customdata = array('username' => "Failed login attempt: ".$username,
