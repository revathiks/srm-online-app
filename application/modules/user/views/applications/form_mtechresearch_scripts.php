...
<script>
	 var url = "<?php echo base_url(); ?>mtechresearch";
    ...
    <?php if($mode_edit) { ?>
      ...
      var url = "<?php echo base_url(); ?>mtechresearch/"+mode_edit_val+"/"+crm_applicant_login_id+"/"+crm_applicant_personal_det_id;
  ...
  var csrfName = '<?php echo $this->security->get_csrf_token_name(); ?>',
    ...
    <?php if($payment_mode_id == PAYMENT_MODE_DEMAND_DRAFT_ID) {
      ...
      ?>
       setTimeout(function() { 
        if(parseInt(total_wizard_Step-1) == <?php echo $startIndex; ?>) {
        	$("#save_exit").remove();
        	//enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>mtechresearch-preview');
          if(mode_edit_url != '')
          {
            enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>mtechresearch-preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
          }else{
            enable_preview_btn(<?php echo $startIndex; ?>,'<?php echo base_url();?>mtechresearch-preview');
          }
        }
      ...
      tab_wise_percentage_show(get_percentage_url,'percentage_bar');
            ...
            $(this).attr('isexit','');
                ...
                if(currentIndex === 5) {
                    ...
                    if(photograph.isValid() && signature.isValid() && graduation_marksheet.isValid()) {
						//var base_url = "<?php echo base_url(); ?>mtechresearch";	
						//var startIndex = currentIndex+1
						//window.location.href = base_url+'?startIndex='+startIndex;
                ...
                var AjaxRequest = $.ajax({
						...
						success: function(returndata) {
							if(returndata) {									
								console.log(returndata);
								var startIndex = currentIndex+1
								window.location.href = '<?php echo base_url().'mtechresearch';?>?startIndex='+startIndex;
							}
						},
						error: function(returndata) {
						  console.log(returndata); 
		...
		if(currentIndex == parseInt(total_wizard_Step - 1)){
			$("#save_exit").remove();
			//enable_preview_btn(currentIndex,'<?php echo base_url();?>mtechresearch-preview');
      if(mode_edit_url !='')
        {
          enable_preview_btn(currentIndex,'<?php echo base_url();?>mtechresearch-preview/'+mode_edit_url+'/'+crm_applicant_login_id+'/'+crm_applicant_personal_det_id);
        }else{
          enable_preview_btn(currentIndex,'<?php echo base_url();?>mtechresearch-preview');
        }
		}else{
    ...
    $("#mtechresearch_form").steps(settings);
  //setsave exit attr	
    $(document).on("click", "#save_btn" , function() {
    	 $("#mtechresearch_form").attr('isexit',1);
         $("#mtechresearch_form").steps('next');
    });
    
    $('.actions a').click(function(){        	 
      $("#mtechresearch_form").attr('isexit','');
    }); 
//end set attr
  //to remove links from previous tabs a
    <?php if($current_wizard>=FORM_WIZARD_PAYMENT_ID && (isset($is_allow_previous) && $is_allow_previous==0) 
     ...
     $( ".steps li" ).each(function( index ) { 
      if(index<5){       	 
   	  $("#mtechresearch_form-t-"+index).removeAttr("href");
     }			   
   	});
    <?php } ?>
        $(document).ready(function () {
			...
			getSelect2('campus_preference1','<?php echo base_url("get_choice_dropdown"); ?>?is_lookup_master=1&is_campus=1&is_distinct=1&grad_id='+const_grad_id+'&deg_id='+const_deg_id+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Choose Campus Perferences 1', formatRepoCommon,no_campus_prefer_msg, false, formatRepoSelection);
			...
			getSelect2('campus_preference2','<?php echo base_url("get_choice_dropdown"); ?>?is_lookup_master=1&is_campus=1&is_distinct=1&grad_id='+const_grad_id+'&deg_id='+const_deg_id+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Choose Campus Perferences 2', formatRepoCommon,no_campus_prefer_msg, false, formatRepoSelection);
			...
			getSelect2('campus_preference3','<?php echo base_url("get_choice_dropdown"); ?>?is_lookup_master=1&is_campus=1&is_distinct=1&grad_id='+const_grad_id+'&deg_id='+const_deg_id+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,'Choose Campus Perferences 3', formatRepoCommon,no_campus_prefer_msg, false, formatRepoSelection);
	...
	function test_campus_pref(campus,campus_placeholder) {
		var no_campus_prefer_msg="Sorry, Campus Preference not available.";
				getSelect2(campus,'<?php echo base_url("get_choice_dropdown"); ?>?is_lookup_master=1&is_campus=1&is_distinct=1&grad_id='+const_grad_id+'&deg_id='+const_deg_id+'&form_id='+app_form_id+'&sort_by=name&sort_order=asc'+url_edit,campus_placeholder, formatRepoCommon,no_campus_prefer_msg, false, formatRepoSelection);
