 ...
 *
 */
class Law extends API_Controller {
	...
	/*Law Save Basic Details*/

	public function law_basic_detail_post()
	{
		...
		$applicant_appln_det_columns = 'applicant_personal_det_id,appln_form_id,qualifyingexamfromindia,is_agree,active,application_status_id';		
		...
		if($applicant_appln_det_res)
		{
			...
			$wizard_update = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_LAW , FORM_WIZARD_BASIC_ID);
		}
		$this->response ($applicant_appln_det_res , API_Controller::HTTP_OK);
	}

	public function law_personal_detail_post() 
	{ 
		...
		$applicant_course_prefer_id = $this->post('course_prefer_id',true);
		...
		$course_pref_arr = explode(',', $course_pref);
		// get course prefer id - prog id
		$applicant_course_prefer_res = $this->_get_prog_id_by_preference_dtl($grad_id, false, $app_form_id, $course_pref_arr);
		$applicant_course_prefer_res = implode(',', $applicant_course_prefer_res);
		...
		//Course start
		$course_pref_res = array();

		if($course_pref){
			$n_course_pref = $applicant_course_prefer_res;
			$n_course_prefer_id = $applicant_course_prefer_id;
			$n_course_choice_no = $course_choice_no;
			...
			if($n_course_prefer_id){
				...
				if(!$chk_flag) {
						$course_pref_res[] = $this->_insert_update_applicant_tables($function_name, array('applicant_personal_det_id'=>$applicant_personal_det_id,'applicant_appln_id'=>$applicant_appln_id), $table_schema, $applicant_course_prefer_table, $applicant_course_prefer_columns, 1, 'applicant_course_prefer_id');
				}
			}
		...
		{
			$wizard_update = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_LAW , FORM_WIZARD_PERSONAL_ID);
		}
	...
	public function law_parent_detail_post() {
    	$app_form_id = APP_FORM_ID_LAW;
		...
		if($parent_res)
		{
			$wizard_update = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_LAW , FORM_WIZARD_PARENT_ID);
    ...
    public function law_acdemic_dtl_post() {
    	$app_form_id = APP_FORM_ID_LAW;
		...
		$stream_name =$this->input->post('academic_stream');
		$other_stream_name = $this->input->post('other_stream_name');
		...
		if($current_education_qual_status == 1) {
		...
		$function_name = $this->get_function_name(__FUNCTION__);
		$applicant_schooling_det_columns = 'applicant_scl_det_id,applicant_personal_det_id,applicant_appln_id,institute_name,board_id,marking_scheme_id,mark_percentage,completion_year,registration_no,name_as_in_marksheet,mode_of_study_id,address,schooling_id,active,result_declared,is_curr_qualify,other_stream_name,stream_id';
		if($scl_det_id) {
		...
		if($school_institute_name) {
				foreach($school_institute_name as $k=>$v) {
					...
					if($stream_name!=OTHER_STREAM){ $other_stream_name=Null; }
					$_SERVER["REQUEST_METHOD"] = "POST";
					$_POST = array();
					...
					if($scl_data == 1 && $current_education_qual_status == 0){
						$n_mark_percentage =NULL;
						$n_marking_schema ='null';
					}
		...
		if($other_res)
		{
			$wizard_update = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_LAW , FORM_WIZARD_ACADEMIC_ID);
		}
    ...
    public function law_declaration_detail_post(){
		...
		if($applicant_appln_det_res)
		{
			$wizard_update = $this->common_wizardupdate($applicant_personal_det_id , APP_FORM_ID_LAW , FORM_WIZARD_UPLOAD_DECLARATION_ID);
	...
	public function upload_final_step_post() {
		...
		$applicant_appln_det_columns = 'applicant_appln_id,applicant_personal_det_id,application_status_id,active';
		//  applicant_personal_det insert / update
