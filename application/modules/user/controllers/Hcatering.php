...
class Hcatering extends FrontendController
{
    //
    public $CI;
    ...
    public function hcma_form($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {   
        $this->is_exists_user_logged();
        /*CRM ADMIN Edit form start*/
        ...
        if ( $this->input->post () ){
            ...
            $user_data['app_form_id']=APP_FORM_ID_HCMA;
            $app_form_id=APP_FORM_ID_HCMA;
            // Basic Details Start
            if($user_data['currentIndex']==0){ 
                // Basic Details API & Method
                $url = $api_urls[ 'hcma_basic_detail' ];
            ...
            if($user_data['currentIndex']==1){
                ...
                $url = $api_urls[ 'hcma_personal_detail' ];
               // $userdata = $this->session_userdata();
                $user_data['userdata'] = $userdata;
                // Get Personal ApplicaNT ID
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            if($user_data['currentIndex']==2){
                $url = $api_urls[ 'hcma_parent_detail' ];
               // $userdata = $this->session_userdata();
                $user_data['userdata'] = $userdata;
                // Get Personal ApplicaNT ID
                $user_data['applicant_personal_det_id'] = $applicant_id;
            ...
            if($user_data['currentIndex']==4){
                $api_urls = $this->config->item ( 'api_urls' );
                $url = $api_urls[ 'hcma_academicdet' ];
                $method = 'POST';
                //$userdata = $this->session_userdata();
                $user_data['userdata'] = $userdata;
                $user_data['applicant_id'] = $applicant_id;
                $user_data['current_qualification_status'] = $user_data['current_qualification_status'];
                $user_data['sslc_result_status_val'] = $user_data['sslc_result_status_val'];
                 
                $current_education_qual_status = $user_data['current_qualification_status'];
                $sslc_result_status_val = $user_data['sslc_result_status_val'];
                if(isset($sslc_result_status_val) && $sslc_result_status_val!=""){
                    $cur_qual_completed=$sslc_result_status_val;
                }else{
                    $cur_qual_completed=$current_education_qual_status;
                }
                $user_data['cur_qual_completed'] = ($cur_qual_completed==1)?'t':'f';
                $user_data['name_as_in_marksheet'] = $user_data['candidate_name'];
                
                // Tenth Academic
                
                $user_data['cur_qual_completed_tenth'] = ($sslc_result_status_val==1)?'t':'f';
            ...
            //end uploads
			   ...
			   if((empty($isValidDate) || $isValidDate=='') && $this->mode != ADMIN_MODE_EDIT  && empty($this->is_private_ip)){
				 redirect(base_url());
			   }              
                //redirect to dashboard after appln completion
                 ...
                 $wizard_dt = $this->_get_wizard_details(APP_FORM_ID_HCMA);
            ...
            $this->data['site_title'] = $this->data['page_title'] = 'Hotel and Catering Management Application Form';
            $this->template('applications/hcma_form', $this->data, true);
        }
    }
    
    public function hcma_form_preview($mode = false , $applicant_login_id = false , $applicant_personal_det_id = false) {
        $this->is_exists_user_logged();
        /*CRM ADMIN Edit form start*/
        $this->data['get_admin_session'] = $get_crmadmin_session = $this->_get_crmadmin_session();
        $this->data['mode_edit'] = $mode;
        ...
        $this->data['user_data'] = $this->user_details_data['user_details']['data'][0];
        $this->data['personal_detail_list'] = $this->personal_detail_list($applicant_id);
        $this->data['site_title'] = $this->data['page_title'] = 'Hotel and Catering Management Form Preview';
        /* applicant_address_details_list end */
        
        // Display page with the template function from REST_Controller
        $this->template('applications/hcma_form_preview', $this->data, true);
    }
  
    public function get_hcma_preference() { 
        $api_urls = $this->config->item ( 'api_urls' );
        $url= $api_urls['hcma_preference'];
    ...
    public function get_hcma_branchs() {
        parent::_get_hcma_branchs(false, $this->is_admin);
    }
    ...
    public function get_cur_edu_status() {
        parent::_get_cur_edu_status(false, $this->is_admin);
    }
    
    public function parent_occupation() {
        parent::_parent_occupation();
    }
    /**
